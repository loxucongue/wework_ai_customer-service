...
客户回复产品只保留 V3；`main` 是唯一长期开发与生产分支。API、worker、frontend 可以拆进程，但最终必须从同一个干净的 main SHA 构建。旧产品 V1/V2 路由永久返回 410，并在合并完成后从代码删除。
...
- 在统一 main release 完成前，V3 sidecar 与 shared API/worker 仍需并行运行。
