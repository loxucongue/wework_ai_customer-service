...
- 可处理的未发送异常统一预警，包括业务不发、顺序阻塞、接口异常、重试中和 Shadow。
- 客户关系已删除、人工接管和发送结果未确认只保留阻断/恢复与审计，不算发送失败，不发送预警。
...
| `humantakeover` | 人工接管 | 当前会话由人工接待 | 阻断，不消费，不预警 |
| `customer_deleted` | 客户删除 | 客户关系已删除 | 阻断，不消费，不预警 |
...
`add_wecom` 固定首次加微任务必须在首次判断、旧版恢复路径、失败重试、管理员手动重发和最终发送前都读取并校验权威会话。加微后出现任意真实客户回复时，任务以 `customer_replied_after_add` 进入失败阻塞且不消费，模型不得恢复发送；企微自动开场白不算真实客户回复。会话响应缺少客户关系或消息列表、请求失败，或加微/回复时间无法可靠判定时，均保守阻断且不得调用发送接口。非 `add_wecom` 任务继续由正常上下文和模型策略判断。
