...
发送阶段的企微聚合平台 `409` 属于不可重试失败，立即只消费对应平台 `taskId` 为 `70` 并回传失败策略数据。网络超时从首次发送失败开始最多重试 600 秒；到期执行同样的失败终态。失败终态不得消费 `/event/trigger/sop-messages` 返回的内容 `msgId`，本地发送任务与 SOP 事件必须同步进入终态。
