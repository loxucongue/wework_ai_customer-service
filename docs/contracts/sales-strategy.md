...
- 确定性排序不是业务结论。最终 Reply 必须复核客户当前原话、权威事实和允许的推进压力，可独立采用普通跟进序列和同卡点话术，也可不采用任何候选。只有话术动作与真实序列节点动作一致时才写入 `sequence_links`；同类型语义候选不得伪装成节点直属话术。
...
- 门店城市/区域查询可保留完整已授权候选用于事实审计：不超过 6 家时全部交付门店卡；超过 6 家时不截断事实候选，由 Reply 输出完整“门店名 + 区县”文字清单。事实生成、Reply 合同与失败恢复必须共同引用同一组 `delivery_store_ids` 或同一份完整文字候选，不能各环节使用不同范围。
