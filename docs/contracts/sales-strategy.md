...
- 客户可见表达默认回答一个当前问题并给一个相关下一步；简单问题通常只发一条，不得输出内部审计口径、客服菜单或同义重复。当前消息高于不可靠旧历史，历史冲突、测试噪声或未真实交付草稿不得恢复旧门店、旧活动或旧订单话题。
...
- 普通跟进序列与卡点话术采用双轨候选：序列节点动作解释策略节奏，话术解决当前客户卡点，二者互相提供排序证据但不再互相卡死。普通话术只以第三方 `checkpoint_type_id` 作为硬边界；本轮 tag、Router 表达焦点和序列节点 action 作为排序加分，不作为过滤条件。查询同类型已发布内容后执行文本相关度排序、相同动作/标签限额、重复内容去除和总字符预算，最终最多向 Reply 提供 6 个段落，不得跨卡点类型补齐。
