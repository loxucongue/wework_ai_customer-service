...
计划任务数不是固定两步：有明确卡点时从所选外部跟进序列中续接尚未实际发送的不同节点，单次最多 3 个解卡动作和 1 个成交承接动作；无明确卡点时等于 DeepSeek 选择的未完成主线来源数。看板必须展示计划模式、真实序列/节点 ID、原始节点时间、归一化时间、夜间调度模式、候选话术、正式采用话术、本轮新价值和成交承接动作。每日计划/任务上限显示为“不限制”，但不能把同一沉默周期指纹幂等或逐次安全阻断显示成限频。
