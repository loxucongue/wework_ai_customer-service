...
| 项目 | 数量 | 约束 |
|---|---:|---|
| 测试直接导入私有符号 | 152 | 禁止增长；修改相关域时必须减少 |
| 涉及测试文件 | 31 | 报告项，不作为独立阈值 |
| `except Exception` | 160 | 禁止增长 |
| 裸 `except` | 0 | 禁止新增 |
| 静默 `pass` / `return None` | 51 | 禁止增长 |
| 其中纯 `pass` | 19 | 禁止增长 |
| 启发式 fail-open 返回 | 16 | 禁止增长，逐项人工复核 |

基线是债务上限，不是认可清单。减少计数后应同步降低基线；不得通过改名、动态导入或放宽扫描范围绕过门禁。fail-open 是保守的语法启发式：异常分支直接返回 `True`、成功字符串或空容器会被标记，不代表每一项都已确认是业务漏洞。

长期测试优先覆盖公开业务合同：角色路由、V3 回复、身份隔离、支付/门店/订单硬事实、SOP 两段式终态、Outreach 发送前检查和送达回调。结构测试只约束少量防回流边界（入口大小、唯一决策阶段、禁止动态代理）；不得为每个私有辅助函数建立兼容测试。现存私有导入不会因本次清理直接删除，修改对应业务域时必须先用公开合同替换，再删除旧测试。
