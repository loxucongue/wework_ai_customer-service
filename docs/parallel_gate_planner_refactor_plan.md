...
本次重构按“先影子、再对比、再灰度、最后清理”的方式推进。任何阶段都不能直接替换线上主链路，除非上一阶段的结构、效果、规则完整性和回滚条件全部通过。

### 25.1 架构校正

目标不是让 Gate 变成新的大脑。目标分工如下：

| 节点 | 目标职责 | 不能做的事 | 失败风险 |
|---|---|---|---|
| SOP Chat Gate | 找到当前消息可引用的 SOP、精准回复和简单话术素材；判断是否需要进入 Reply 或工具链路 | 不最终判断客户意向、不决定成交动作、不生成工具参数、不写 SOP 进度 | Gate 越权会导致客户已延期还继续追问、客户有异议却机械发包 |
| Tool Planner | 只判断本轮回答缺哪些动态事实，并生成最小只读工具计划 | 不写客户话术、不判断心理、不决定主线推进、不执行写工具 | Planner 继续当销售大脑会让 Reply 失去最终控制 |
| Reply | 最终理解完整聊天、客户最新表态、Gate 内容候选和工具事实，生成唯一客户可见回复 | 不编造工具事实、不绕过硬校验、不自行假设支付或门店事实 | Reply 输入不足会继续出现答非所问、重复追问、机械推进 |
| Code/Join | 合并分支、结构校验、幂等、硬边界、失败恢复 | 不用关键词判断普通销售心理和阶段 | Join 变业务引擎会违反项目宪法 |

并行的价值是提前准备“内容候选”和“工具事实”，不是把最终判断拆给多个节点。复杂场景最终只允许 Reply 做一次业务判断。

### 25.2 阶段提交计划

每个阶段独立提交到 `codex/reply-chain-refactor`，不提交 `main`，不部署。

| 阶段 | 开发目标 | 允许改动 | 禁止改动 | 交付物 |
|---|---|---|---|---|
| D0 基线冻结 | 固化当前真实链路输入输出和效果基线 | 报告、fixture、只读脚本 | 改 Prompt、改业务规则、改运行时 | baseline JSON/MD、测试结果 |
| D1 Authority Snapshot | 建立完整带时间聊天和权威事实快照 shadow | 新 schema、shadow builder、隔离测试 | 替换正式 Planner/Reply 输入 | snapshot diff 报告 |
| D2 Gate Preview 分离 | Gate 只生成 preview，不提前写 SOP task/send_once | Gate 输出结构、commit 协调 shadow | 改 SOP 业务顺序、改发送协议 | preview 与旧输出对比 |
| D3 Gate Router v2 Shadow | 新 Gate 输出内容候选和路由建议 | Gate prompt/schema/test | Gate 直接处理复杂历史状态 | 单节点 Gate 报告 |
| D4 Tool Planner v2 Shadow | Planner 只输出工具计划 | 工具计划 schema、只读工具映射 | Planner 输出客户文案或成交动作 | 工具计划准确率报告 |
| D5 并行 Shadow | Gate 与 Tool Planner 使用同一事实快照并行计算 | runner、trace、diagnostics | 写数据库、发消息、提前开单 | 冲突率和耗时收益报告 |
| D6 只读工具提前执行 | 只读工具按白名单提前执行 | read-only executor、权限校验 | 写工具提前执行 | 工具事实合同报告 |
| D7 安全直回灰度 | 只放开极少数简单直回 | direct validation、flag | 让 Gate 处理软拒绝、付款、门店异议 | 直回误判率报告 |
| D8 Reply 输入切换 | Reply 接收 Gate 候选 + 工具事实 + 完整聊天 | Reply payload、prompt、validation | 删除旧链路字段消费者 | 新旧 Reply 对比报告 |
| D9 Reply 业务所有权切换 | 最终客户状态、轮次结果、主线动作归 Reply | Reply schema、旧字段 shadow | 一次删除旧 normalizer 保护 | 业务决策差异审核 |
| D10 写工具分离和清理 | 延后写操作，清理旧字段 | deferred write executor、旧字段删除 | 改外部 API 或消息 schema | 清理清单和回归报告 |

### 25.3 每阶段 Review 安排

每个阶段至少两轮 review。

第一轮是代码结构 review，重点看是否偏离项目宪法：

1. 是否新增 Python 关键词分支判断普通销售意图、顾虑或阶段。
2. 是否让 Gate、Tool Planner、Join 接管最终客户心理或成交动作。
3. 是否在 shadow 或并行阶段执行了写工具、创建 SOP task、send_once 或真实发送。
4. 是否改变平台请求/响应 schema、客户可见消息 schema 或外部接口协议。
5. 是否仍以 `corp_id + wechat + customer_id/external_userid` 隔离销售接触档案。
6. 是否保留已付、健康风险、投诉退款、门店权限、金额和同轮单卡等硬边界。

第二轮是业务规则 review，重点看是否丢规则或降效果：

1. 本阶段删除或移动的每条 Prompt/规则是否进入 `rule_ownership_matrix`。
2. 每条 active 规则是否有目标 owner、事实依赖和回归测试。
3. 完整带时间聊天是否真实进入模型输入，而不是被旧画像覆盖。
4. 当前客户最新原话是否高于历史画像、旧策略摘要和旧未完成槽位。
5. Reply 是否仍能做到“先回答当前问题，再选择一个合理主线动作”。
6. 对延期、拒绝、工作中、已付后时间不定等场景，是否允许合理停留，而不是机械推进。

高风险阶段 D2、D7、D8、D9 必须由另一个 Codex 窗口按提交 hash 做只读 review。review 先列 findings，再决定是否继续。

### 25.4 测试节点步骤

测试按从确定性到模型效果逐层放大，不能只用总通过率判断。

| 测试节点 | 运行时机 | 验证内容 | 通过标准 |
|---|---|---|---|
| T0 静态门禁 | 每次提交前 | `git diff --check`、`py_compile`、UTF-8/乱码扫描、schema 序列化 | 100% 通过 |
| T1 合同测试 | D1-D4 每次改 schema | 新旧字段兼容、shadow 不污染正式输入、模型 payload 不含 shadow 内部字段 | 100% 通过 |
| T2 Gate 单节点 | D3 起 | SOP/精准回复/简单场景命中，复杂场景进入 Reply | 关键场景 5/5 |
| T3 Tool Planner 单节点 | D4 起 | 工具能力、参数、只读/写分类、工具不足反问目标 | 关键场景 5/5 |
| T4 Join 确定性 | D5 起 | Gate 路由和工具事实如何合并，Planner required 是否否决直回 | 100% 通过 |
| T5 Reply 单节点 | D8 起 | 当前问题、历史承接、主线动作、真人感、事实安全 | 总体 >=90%，关键场景 5/5 |
| T6 并行组合 | D5-D8 | 分支先后完成、超时、repair、取消、hedge、浪费调用 | 无写操作、无空结果 |
| T7 离线仿真 | D6 起每阶段 | 多轮状态污染、SOP 节奏、门店、支付、风险、软拒绝 | 硬错误 0，关键场景全过 |
| T8 新旧链路对比 | D8-D10 | 相同输入下旧链路和新链路差异、P50/P90、token、fallback | 效果不低于基线 2 个百分点 |
| T9 隔离 smoke | 全部通过后 | 使用隔离客户验证接口、日志、持久化、卡片结构 | 无真实客户发送 |

### 25.5 必测业务轨迹

以下场景必须进入仿真和单节点测试集，避免只测“标准顺序客户”：

| 类别 | 轨迹 | 主要风险 |
|---|---|---|
| 门店 | 省、市、区县、县城、乡镇、村、地标、定位卡、同城多店、不可见门店 | 编造门店、发不可见门店、重复问方便不方便 |
| 活动/价格 | 问活动、问总价、质疑隐形消费、问是否一次性、问效果图 | 没有先答问题、活动包触发错误、没有回主线 |
| 预约金 | 活动已铺垫后问怎么付、软拒绝距离、明确参加、重复发卡、同轮多卡 | 恢复旧订单前置、重复卡、机械压单 |
| 已付后 | 收姓名电话、时间不确定三次、改天、只确认到店意向 | 已付后继续发卡、重复问时间 |
| 风险/拒绝 | 过敏发炎、投诉、退款、明确不要、正在工作 | 继续营销、SOP 主动触达误发 |
| SOP | 只破冰沉默、轻触后未回、需求案例未发、活动已讲过、send_once 冲突 | 越过主线、重复破冰、候选包和历史重复 |
| 多轮理解 | 客户切换问题、回答过斑点时长、从门店切到付款、从效果切到价格 | 旧槽位压过当前消息 |
...
1. 建立 `rule_ownership_matrix`，每条 active 规则必须有 `target_owner` 和 `regression_tests`。
2. Prompt 精简只能合并重复表达，不能删除唯一语义。删除前必须能指出迁移位置。
3. 业务规则提交和架构搬迁提交分离。架构提交不得顺手改价格、退款、项目范围、门店口径或预约金规则。
4. 每次迁移后跑同一批基线场景，报告必须包含“改善、持平、退化、波动、不可评估”。
5. 如果模型效果下降，优先补模型上下文和 Prompt 分层，不回到 Python 关键词业务分流。
6. 如果并行收益不足或效果下降，保留串行链路，不为速度牺牲回复质量。

### 25.7 停止条件

出现任一情况立即停止本阶段，不进入下一阶段：

- 硬错误不为 0。
- 关键场景不是 5/5。
- 新链路总体语义通过率低于基线 2 个百分点以上。
- Gate 直回误判复杂场景。
- Tool Planner 输出客户可见文案或销售心理判断。
- Reply 没有拿到完整带时间聊天。
- 任何 shadow 路径写数据库、创建 SOP task、send_once 或真实发送。
- 出现不可见门店、已付后发卡、健康风险营销、同轮多预约金卡、空回复或中性兜底泛滥。

## 26. 当前下一步

当前分支已经完成部分 shadow 基础设施。下一步不直接改客户可见行为，优先完成：

1. 把完整带时间聊天和 authority snapshot 固化为可审计 shadow 输入。
2. 建立 `rule_ownership_matrix` 初版，先覆盖预约金、门店、SOP、精准回复、已付、风险六类规则。
3. 给 Gate/Tool Planner/Reply 分别补 payload isolation 测试，确保 shadow 字段不会污染正式模型输入。
4. 扩充离线仿真测试集，优先覆盖最近线上暴露的问题轨迹。
5. 每完成一个阶段独立提交到 `codex/reply-chain-refactor`，提交后先 review 再进入下一阶段。
