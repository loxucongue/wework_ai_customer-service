...
| 3 | 并行查询 `conversation/status` 与 `conversation` | 取得 `ai_auto_reply`、客户关系和消息列表 | 任务70，无 `messages`，回传失败策略数据 |
| 4 | 判断未开口、未删除、AI托管 | 三项全真才继续 | 任一不满足：任务70，无 `messages`，回传具体原因 |
| 5 | `POST /event/trigger/sop-messages` | 按 `eventLogId` 取得 `nextGroup` | 任务70，无 `messages` |
| 6 | 校验第一组 | 必须有 `id/msgId` 和至少一条合法消息 | 任务70，无 `messages` |
| 7 | `POST /api/v1/platform-agent/ai-outreach/send` | 原样发送整组内容；接口正常返回即进入成功路径 | 接口直接报错：任务70，无 `messages` |
...
失败请求不得出现 `messages`，成功请求只能出现本轮实际调用发送接口的一条内容组结果。

## 运维核对

- 看到任务30时，消费审计中必须同时有且仅有一个 `messages[].msgId=30`。
- 看到任务70时，请求中必须没有 `messages`。
...
- 客户删除和人工接管不发钉钉失败预警；其他未发送异常按告警规则执行。
