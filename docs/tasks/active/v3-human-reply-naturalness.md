...
### 2026-09-10 母窗口退回后的受控消融轮（最新）

- 母窗口退回 `d145fbe4`；本轮不得合并、部署或启动人工盲审。保留 `a924b098`、`d145fbe4` 已推送历史，不 reset、不改写历史。
- 启动现场：`HEAD=d145fbe46e5489e031b88f824fe7fd1e86f56731`，dirty=false，`origin/main=afb261c0480811c89fd58f98c3c778169303f287`。
- 先做 V0–V3 受控消融，不直接继续修改正式 Prompt。冻结同一 90 场景、DeepSeek、temperature=0.15、候选上下文顺序与评分函数；先跑上一轮 12 个失败及其正反配对，每个变体每场至少 3 次。预筛同时要求无关插入 0、业务认可推进不低于 V0、首次软拒绝不错误 keep_open、明确请求不被误伤、无未经事实支持的真实完成态。
...
- 处置：通过新增提交把正式 Reply Prompt、动态主线指令、事实主题范围、话术说明、架构快照和对应候选测试恢复为 `origin/main` 行为；保留已推送历史，不 reset、不改写。保留独立的受控消融工具、完整图测试基础设施、产品合同边界和本轮证据。
- 恢复后核验：正式 Prompt 为 6,995 字符、73 行、SHA-256 `fa3885a8c0aea01bebbf4eac7e91a61c5193850c4a5336ca213e015a5705475b`，与 `origin/main` 逐字一致；Prompt、Reply 上下文、架构快照和治理测试相对 `origin/main` 均无差异。Ruff 与 `git diff --check` 通过；全量 L1 为 866 passed、9 个既有弃用警告。
