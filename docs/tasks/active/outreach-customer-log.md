...
- 后端接口保留 30 天默认、90 天上限；页面首屏使用近 7 天，并保留 30/90 天按钮。实现客户身份隔离、无计划事件去重和实际发送凭据校验。
...
- 列表 contact key 附带当前窗口内同一外部联系人的全部已确认平台客户 ID 作为索引提示；详情先按现有 `customer_id` 索引缩小范围，再强制复核企业、企微和外部联系人，13 组历史多 ID 映射不会因优化漏读。
- 计划详情读取关联沉默运行时关闭不必要的计划/任务递归装载，不改变返回内容。
...
- 修复发布后经前端代理只读实测：30 天完整列表 18.22 秒，7 天完整列表 2.31 秒；因此页面首屏固定为近 7 天，历史大范围仅按需查询。
- 首轮下钻基线为客户详情 15.40 秒、计划详情 6.80 秒；索引提示和递归读取修复将在最终 release 发布后复核。
