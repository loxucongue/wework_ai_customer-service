...
- L1 定向合同：121 passed。
- L1 最终全仓：438 passed；仅现有第三方依赖弃用 warning。
- Prompt 长度：8918，低于 9000 字符预算。
- L2 DeepSeek 指定日志真实身份只读复现：运行异常 0；主模型回复；序列 11、话术 225 均采用；效果视频同轮交付；无门店重复查询/重复门店卡；无兜底。
- L2 DeepSeek AI 初评：1/1 通过、真人表达 1/1、意图/情绪一致 1/1；完整生命周期 12.989 秒，图后 48ms。
- 隔离审计：公网回复接口未调用、生产 repository 未传入、外部写阻断器 7 个、生产 runs/usage/dispatch/outbox 反查均为 0、写尝试 0；模型仅 `deepseek-v4-flash` 与 `deepseek-chat`。
