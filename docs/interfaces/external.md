...
- 已观察到的动作码：
  ...
  - `act017` 项目说明 S10N
  - `act018` 项目说明 眼袋 m10
  - 当前租户目录还实际使用 `act019～act038`。这些名称和含义由平台目录返回，AI Paths 不维护第二份本地语义映射；客户端接受规范的 `actNNN`，并保留序列全部合法节点，避免平台新增动作后被静默丢弃。
- 运行边界：
  ...
  - 当前本地临时目录版本为 `2026-09-05.business-workbook-real-deal-v1`，来自业务真人成交版收集表，包含 9 条规则、16 套策略、37 个节点类型和 42 条话术。金额、有效期、退款、门店、交通、档期和支付入口等字段仍必须由本轮权威事实支持。
...
- 运行边界：
  ...
  - 可处理的未发送异常发送钉钉预警；客户关系已删除、人工接管和发送结果未确认不算发送失败且不预警。预警失败不改变任务状态。
