...
- 合同文档：[消息送达回调](../contracts/message-delivery-callback.md)
- 鉴权：回调 token。
...
- `GET /admin/sop-platform-dashboard`
...
- `GET /admin/outreach/customer-logs`
- `GET /admin/outreach/customer-logs/{contact_key}`
- `GET /admin/outreach/customer-logs/{contact_key}/plans/{plan_id}`

`POST /admin/sop-platform-tasks/{task_id}/resend` 仅为历史调用兼容保留，当前确定性 SOP 状态机固定返回禁用结果，不能用于人工补发、改变终态或绕过新的 pending 任务。调用方不得把它展示为可执行操作。
...
- `GET /admin/outreach/first-day-settings` 返回的关键运行信息包括启用状态、沉默分钟数、企微范围、启用水位、动态任务来源、每日计划/任务限制，以及夜间开始、结束、恢复和活跃压缩窗口。Worker `/health` 额外返回实际目标扫描间隔、连续失败数和下一次等待秒数；具体秒数属于动态配置，页面必须以 Worker 现场值为准。历史 `first-day` 路径只为接口兼容。
...
- `GET /admin/outreach/customer-logs` 默认查询 30 天、最大 90 天，支持企业、接待企微、客户 ID、外部联系人 ID、自动来源、计划/任务状态和原因筛选。返回客户级计划数、任务处理数、实际发送数和下一任务；`contact_key` 只编码销售接触边界，不作为新的客户事实。
- 客户列表会在数据库内按客户、自动来源和原因聚合重复的未建计划扫描事件；聚合只减少返回和处理行数，`no_plan_count` 仍表示原始事件次数。单客户详情不使用聚合结果冒充历史，而是按完整身份边界读取真实逐条记录。
- `contact_key` 是服务端生成的短期分页/下钻游标，可携带当前列表已确认的平台客户 ID 作为索引提示，但调用方不得解析或自行拼接。详情服务仍按企业、接待企微和外部联系人复核，平台客户 ID 不构成身份替代。
- 客户详情和计划详情继续校验 `corp_id + wechat + external_userid`；仅当记录没有外部联系人 ID 时，才以 `customer_id` 形成独立兼容边界，禁止把两类 ID 互相替代。计划详情展示现有客户、外部联系人、加微关系、接待人员、企微、企业和会话 ID，历史缺失显示“未记录”。实际发送必须同时满足任务 `status=sent` 和已记录平台消息 ID；只有状态没有消息 ID 的任务单独提示待核验。
- 管理前端统一使用 `/logs/outreach`；历史 `/logs/outreach-first-day` 永久跳转。页面按客户、计划时间线和任务证据三栏展示，不包含人工计划，不触发模型、计划或发送。
