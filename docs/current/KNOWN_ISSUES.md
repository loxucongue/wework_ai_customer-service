...
5. 统一销售决策与 Reply 精简代码完成后仍需 400 条业务人工金标 A/B 验收；未达到质量、安全和延迟门槛前，`AI_SALES_POLICY_ENABLED` 必须保持 `false`。下一次部署必须先备份数据库并执行 Alembic 到 `20260904_01`，再以同一 clean main SHA 重启 reply、control 和 worker。
