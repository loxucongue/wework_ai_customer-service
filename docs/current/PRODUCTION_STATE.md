...
2026-09-16 发布客户接待状态持久化接口：生产 schema 从 `20260910_01` 升至 `20260914_01`，新增 `aics_reception_states`、`aics_reception_events`、`aics_reception_relations` 三张空表。随后按产品决策移除企业、员工映射和来源 IP 白名单，配置独立生产 Token；公网无 Token 返回 401，正确 Token 加无效请求返回 400，三张表仍为 0 行。接口以 `corp_id + employee_wechat_id + external_userid` 隔离状态，持有 Token 的调用方对身份真实性负责；尚未让 Reply、SOP 或主动唤醒改用本地状态。回滚点为 `ai-paths-unified-20260916-reception-93a99f5c`；迁移是加法，应用回滚保留新表。公网当前仅 HTTP，应尽快增加 HTTPS 或可信专网，避免 Bearer Token 明文传输。
