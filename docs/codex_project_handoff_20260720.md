...
> 历史说明：本文档记录 2026-07-20 时点的项目状态。涉及“订单必须先于预约金卡”或“未知消息仍需二次确认”的内容已被当前业务规则取代；现行规则以 `AGENTS.md` 和 `docs/business-rules/current_business_rules_20260722.md` 为准。
...
- 当前有效规则：活动报价已完成/已铺垫后，Planner 可按成交节奏直接发或重发本轮 `payment_collection`；订单不是发卡前置。
- 支付后先收姓名电话，再尝试创建或关联后台订单；后台关联失败不能改变已付事实，也不能让客户回复为空。
...
- `【未知消息类型】` 是平台约定的权威转账成功事件，输入归一后直接作为已付事实处理。
...
- 旧版“订单关联是发卡硬前置”已经废弃；当前是活动报价完成后可发卡，支付后收姓名电话并关联订单。
