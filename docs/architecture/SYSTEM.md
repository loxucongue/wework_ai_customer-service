...
文本链当前固定由 `deepseek-v4-flash` 承担 Semantic Router、`deepseek-chat` 承担最终 Reply 及其唯一一次结构修复/完整重试；Reply tier 不继承全局 GPT 应急候选。连续客户消息按原始顺序作为本轮正文输入，内部合并说明只留审计；最终 Reply 只读取最近 12 条真实客户可见消息，排除未交付草稿、覆盖请求和内部 trace。门店目录只在 Router 明确要求门店事实后加载；城市/区县列表候选不超过 6 家时全部交付门店卡，超过 6 家时保留完整候选并让 Reply 按“门店名 + 区县”输出文字清单。精确位置的最近门店排序仍使用有限候选，不把全部远近门店当作同级推荐。
