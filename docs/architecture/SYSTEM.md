...
沉默计划不再固定两步、15～20 分钟或场景组合。DeepSeek 先依据真实客户原话判断是否有明确卡点：有卡点时从外部 Follow Knowledge 选择一条已发布序列，代码按完整节点数 1:1 物化任务；无明确卡点时由模型从尚未交付的主线 SOP 中选择必要任务。平台节点动作只表示节奏，不是话术硬过滤条件；每个节点最多提供 6 条“同卡点同动作 → 同卡点其他动作 → 全目录语义相关”的候选，发送前 DeepSeek 必须选择真实话术 ID 或记录不采用原因。计划和任务不再受每客户每日 2 个计划/4 个任务限制，但同一沉默周期仍只允许一个有效计划，且每次发送前安全门禁不变。
