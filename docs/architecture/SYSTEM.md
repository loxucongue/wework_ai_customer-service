...
- SQL 安全检查只接受已验证的单语句形态：锁定读保留只读分类，实际单表写入必须指向 `aics_`；源表写、多目标写、可执行注释、多语句、DDL 和歧义转义均拒绝。不关闭 guard 来解决锁定读兼容。
- 同一 URL 的新字节与已有指纹矛盾时必须显式纠错，不能沿用旧身份覆盖指纹；视频转码仍可由同一可信 file ID 证明身份。无字节同步保留已有指纹和人工纠错原因。人工 canonical ID 仅允许小写 ASCII 字母、数字、连字符、下划线和冒号，长度不超过 96，避免 MySQL 大小写不敏感排序规则与 SQLite 的身份等价差异。
- 日志/管理读取使用脱敏副本，隐藏 URL query/fragment、素材字节与内部重放载荷；素材 trace 只称响应提交。核心稳定响应与恢复原件保留完整 URL，不被审计脱敏改写；不得拿审计副本执行发送。
