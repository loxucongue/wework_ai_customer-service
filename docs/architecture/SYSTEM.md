...
沉默唤醒日志的只读观测链按“客户上下文 → 卡点/主线判断 → 跟进序列 → 节点话术候选/采用 → 计划任务 → 发送结果”展示。任务数量动态取决于所选序列节点或主线来源；候选、采用与实际发送必须分开。列表只返回轻量业务摘要；详情从已有运行快照和任务记录派生业务视图，不重新查询当前目录冒充历史事实，也不增加计划链写入或数据库表。
