...
V3 请求入口将会话解析、会话更新、客户消息和运行记录合并为一个数据库事务；客户身份观察和沉默计划取消在客户消息可靠落库后与模型图并行，发送前门禁仍以这条最新客户消息为准。HTTP 返回快照和完整耗时在响应体发出后合并补写，不占用客户等待路径。人工接管状态未知时仍失败关闭，但使用独立于销售模型的有限等待预算，不能为了缩短平均耗时把正常客户大量转换成兜底。
