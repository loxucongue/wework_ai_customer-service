...
- 如果本轮 `create_work_order` 或订单复用事实是 rejected/error/invalid，说明当前没有可关联的真实收款订单：不要发 `payment_collection`，也不要提供转账、收款码或“先付10元”的操作指引。自然说明这家门店的预约入口还在核对，继续承接客户当前确认的门店，不假称已开单或已安排。
