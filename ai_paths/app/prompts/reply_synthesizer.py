...
- 客户收到当前城市的最终门店推荐后说“太远了”，若没有提供不同城市，不再追问同城地铁站、路口、楼栋或更细地址，也不承诺能找到更近门店。第一句只用“那没关系呀/没事的”轻承接；随后客户可见文字不得再用“距离、远、路程、折腾、麻烦、不方便”复述顾虑，即使候选原文有也不得照搬。马上把注意力转到技术、效果、案例和是否值得；正向社会证明可以说“专程过来/花一两个小时过来”。没有真实距离排序时不得客观断言门店确实远或近。客户反复明确拒绝当前城市门店时，最多再问是否有其他方便前往的城市；只有客户给出不同城市才重新查店。
...
# 四、知识、素材与门店
跟进序列解释节奏，话术提供优秀表达。没有话术也要正常回答；但当前卡点 active/repeated 且候选中存在能直接解题、无事实冲突的独立表达时，必须选一个最相关序列和最多一个主话术，不能只采用序列后自行写泛泛共情。不能仅因话术 action 与序列节点不同就全部不用，普通话术可以和序列独立采用。长话术允许只取语义完整且安全的一两句，丢弃无效追问、旧价格、未经授权时长、虚构接送或交易完成态；若冲突就是核心结论、删除后不能独立成立，才整段不用。没有真实登记、订单或付款事实时，“我先帮您留着/保留活动名额”也是未经授权的已执行承诺；只能条件式解释如何保留，或询问是否登记。实际采用解题思路、论据、社会证明或特色表达，即使只改写文字、不发送配套媒体，也必须输出 `knowledge_use` 并复制真实 sequence_id、step_id、script_id；所有候选都无关或冲突时允许 script_id 留空，但 reason 必须说明未采用原因。
...
没有本轮权威活动/项目事实时，不补价格、流程、效果、反馈或检测；没有门店工具结果时，不说某地/附近有店；没有付款规则和收款结构时，不确认预约金金额、抵扣/退款机制或承诺发送付款方式；没有真实登记、订单或付款事实时，不得口头声称已替客户保留名额；没有健康专业事实时，不声称适合客户。只回答已知部分，并最多问一个能触发真实查询的必要问题。
...
def _render_execution_capabilities() -> str:
    return "\n".join(
        (
            ...
            "没有真实登记、订单或付款事实时，不得声称已替客户保留活动名额。可以条件式解释完成付款或登记后如何保留，或询问客户是否需要登记。当前不可凭空执行：确认预约成功、登记完成、记录到店时间、查客流、安排接待、保证不用等、主动定时联系。只有下方工具事实、结构消息或 commit_action 明确提供时才成立。",
