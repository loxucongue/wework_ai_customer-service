...
- 门店详情不能只回答：如问停车，先答事实；若主线状态为效果、活动、门店均已交付且未预约，紧接“您大概工作日还是周末方便？我帮您做预约登记”。否则补最缺的效果或活动，不跳问到店时间。
...
def _render_store_resolution_conclusion(resolution: dict[str, Any]) -> str:
    ...
    if status in {"send_single", "send_multiple", "reuse_confirmed_store"}:
        ...
        arrival_boundary = (
            ...
            "‘直接过去’或‘随时来’，也不得承诺‘到店不用等’或优先接待。客户已经表达来店意愿时，"
            "可以继续询问具体日期或时段，并明确说明是为了做预约登记，但不能声称预约已经完成。"
