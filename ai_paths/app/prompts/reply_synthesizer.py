...
门店查询只证明位置需求和本轮返回的公开门店事实，不等于报名、预约、可直接接待或今天能做。地点不足时只追问客户尚未给出的城市、区县、道路或地标，不猜区域；查询无候选或不完整时如实按工具结果说。预约与到店时间默认可以协调，正常销售轮次可以主动询问客户方便的日期或时段；客户给出具体时间后可以说“这个时间可以协调、先作为到店意向”。门店卡/地址和时间意向都不等于预约完成，没有权威安排时不得说“直接过去、随时来、已有空位、档期已确认、已留位、已登记、已安排”。客户给姓名、电话、意向时间只证明已提供信息，不证明系统或门店已登记。
    ...
    if status in {"send_single", "send_multiple", "reuse_confirmed_store"}:
        ...
        if is_reuse:
            return (
                ...
                "如果客户本轮没有新卡点、暂缓、拒绝、投诉或健康风险，回答后必须发起预约收口："
                "询问大致到店日期或工作日/周末偏好，并直接说明将按客户方便的时间做预约登记；"
                "同时说明活动名额需要用10元预约金锁定，但本轮不得主动发送预约金卡，也不得声称"
                "预约或名额已经保留成功。本场景的最低完成标准是‘回答详情＋预约时间问题＋预约目的’，"
                "只回答详情、只问时间，或使用‘我先记一下、方便后续登记’等不执行本轮预约收口的表达，"
                "都属于未完成。停车类详情只保留停车结论或停车场名称，不补道路、门牌和导航描述；"
