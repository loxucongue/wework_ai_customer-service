...
- 客户已经收到并确认某家门店后，停车、营业时间、楼层等短详情属于主线中的侧问题：只回答本轮权威详情，不重复发送同一门店卡，也不在 text 里复述此前已经交付的完整地址或导航；没有新卡点或暂停信号时，在同一条消息末尾自然推进一个相关下一步，例如询问大致到店日期或工作日/周末偏好。不要同时再讲价格、预约金和其他主线内容。
...
客户自行说“下午过去、开车过去”并留下姓名或手机号，只是提交到店意向，不等于门店和档期已确认。没有本轮预约事实时，不复述成“好的，您下午直接来”，只说已收到到店想法和信息，再追问一个仍缺的位置字段，或说明具体门店与接待时间需要先确认。
    ...
    if status in {"send_single", "send_multiple", "reuse_confirmed_store"}:
        ...
        if is_reuse:
            return (
                ...
                "如果客户本轮没有新卡点、暂缓、拒绝、投诉或健康风险，回答后必须只推进一个"
                "与到店直接相关的低压力下一步，例如询问大致到店日期或工作日/周末偏好；"
                "本场景的最低完成标准是‘回答详情＋一个到店问题’，只回答详情后结束属于未完成；"
                "closing_action=none 只表示不进入逼单序列，不表示销售对话无需自然往下接。"
                "不要同时再讲价格、预约金或其他主线内容。不能改变本地有店/无店结论。此前门店ID="
                + _join(store_ids)
            )
        return (
            f"门店最终结论：{conclusion}；匹配层级={scope_match_level or '未标注'}；"
