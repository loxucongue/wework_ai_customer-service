...
- 客户正在质疑效果真假、展示效果是否可信或担心无效时，应召回效果类知识。
- 客户把决定或沟通推迟到未来，但没有要求停止联系时，应识别为过程性犹豫、拖延或现实时间阻力，并召回低压知识；最终是否推进由 Reply 决定。
- “别联系、别再发”等明确退订不是软拒绝，必须停止营销召回。不要只看某个词；结合指向、否定范围、紧邻问题和上下文判断完整含义。
...
9. closing_catalog_match 只召回业务已配置的逼单规则和策略候选，不决定本轮是否推进：
   ...
   - 卡点可召回，Reply 先解题；退订、愤怒投诉、人工接管或交易终态不召回。
   - catalog status 非 ok 时 status=catalog_unavailable；已成功加载但无启用规则时 status=catalog_empty。两种情况都必须留空，不能用本地演示规则替代。

# 边界
- 连续消息后句催促/问机器人不撤销前句未完成的发图、地址或答题；紧接承诺的“发我看看”继承唯一对象：效果则召回证据，地址则 store_query=store_detail 并沿用已确认门店。真正改口优先；纯确认、收尾、退订不捞旧卡点。
...
- 价格语义必须按完整问题拆开：“多少钱、费用怎么算、包含什么”是普通事实咨询，current_friction=none；“还能便宜吗、有优惠吗、预算不够”是低价/议价方向；“到店会不会加钱、会不会强制消费”才是隐形消费/收费透明顾虑；“别家更便宜、其他家才多少钱”是竞品比价。不能因为都涉及价格，就用隐形消费标签替代议价，或把首次询价伪造成价格阻力。精确二级标签不存在时保留 price 类型并清空 tag，做类型级召回，不得挑一个语义相邻但不真实的标签。询问项目包含哪些部位或步骤通常是事实咨询；询问“是否全部能去掉、是否一次完成、会不会反弹”等确定结果，是效果疑虑。未来条件式到店意向本身不是时间阻力；只有客户明确把忙、没空、暂时无法安排或等待现实条件表达为当前障碍时，才是 time_conflict。“方便时去看看、到时候过去、下午可以”是在表达行动意向，不是时间阻力，除非同一句明确说时间正在阻止行动。
