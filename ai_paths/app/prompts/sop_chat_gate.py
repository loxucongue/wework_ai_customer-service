...
阅读当前消息和完整带时间聊天，从 `content_assets` 中提名 0-2 个资产：一类直接提供当前问题所需证据，另一类可在当前问题被工具或 Reply 答清后提供一个相邻决策价值。资产是事实、证据和结构素材的组合，不是成品回复模板；也可能是从历史优秀销售经验蒸馏出的证据策略。
...
2. `direct` 表示资产本身直接提供当前问题所需的证据；`supporting` 表示它不替代当前答案，而是在当前问题答清后提供一个不同决策维度的新价值。最多一个 `direct`、最多一个相邻 `supporting`，总数最多两个。同一效果问题下的第二份效果证据、同一地址问题下的第二份地址说明，都仍属于当前维度，不能标成相邻 `supporting`；若本轮工具已经提供当前维度的真实证据，也不能再用同维度资产占用相邻价值名额。
...
11. 必须把检索拆成两个互不替代的步骤：
   ...
   - B. 相邻价值检索：假设当前问题将由 Tool Planner 或 Reply 用真实事实答清，再独立检查是否有一个尚未交付、能帮助客户继续判断的跨维度资产。它必须与当前问题以及 A 的直接证据属于不同决策维度；例如当前效果问题已经由案例或效果资产回答，B 只能再考虑活动、地址或真实卡点中的一个，不能再提另一份效果信心策略。若当前不属于明确工作中/要求稍后、健康风险、投诉退款、强拒绝或停止联系等暂停边界，并且确有合适资产，默认提名一个 `supporting`；不能因为 A 没有直接资产，就把 B 一起判空。
   相邻价值不要求客户当前主动询问该资产，也不要求客户先表示同意；它可以由当前消息引用作为本轮衔接上下文，但不能虚构客户对该资产已有顾虑。必须结合完整历史和资产自己的 `customer_uncertainty/useful_evidence/reasoning_moves/anti_patterns` 选择，不得按“门店后固定发活动”等场景映射。Gate 只扩大 Reply 可选择的证据范围，最终是否采用、采用哪个维度以及是否暂停仍由 Reply 决定。只有处于暂停边界，或目录里确实没有尚未交付且有意义的新价值时，B 才能留空。
...
- 提名一个相邻价值资产不等于决定推进；不得输出“门店后发活动”等固定顺序，也不得为了凑满两个候选机械补资产。
