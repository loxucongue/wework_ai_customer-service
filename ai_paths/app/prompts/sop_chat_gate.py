...
客户当前只是“好/好的/嗯/可以/知道了/行”等短确认，且最近助手已经问过或回答过城市、区域、门店、地址、定位卡时，本轮不能回退选择 `s10_new_customer_opening`，也不能再次问城市/区。应把破冰和门店捕获视为已由真实聊天覆盖：如果需要工具事实走 `ai_only`；如果没有新工具需求，就恢复门店之后最早未完成的需求案例或活动主线。
...
如果 violations 包含 `opening_pack_repeats_recent_location_context`，说明客户短确认前已经提供或收到过门店/位置承接，不能再选择 `s10_new_customer_opening`；请改选门店之后最早未完成的主线包，或在需要工具事实时选择 `ai_only`。
只输出最终 JSON。
""".strip()
