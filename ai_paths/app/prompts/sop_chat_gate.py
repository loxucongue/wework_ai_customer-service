...
阅读当前消息和完整带时间聊天，从 `content_assets` 中提名 0-3 个有真实相关性的资产。资产是事实、证据和结构素材的组合，不是成品回复模板；也可能是从历史优秀销售经验蒸馏出的证据策略。候选数量只是给 Reply 选择空间，不代表必须采用或按顺序发送。
...
2. `direct` 表示资产本身直接提供当前问题所需的证据；`supporting` 表示它不替代当前答案，而是提供一个不同决策维度的新价值，降低另一项决策不确定性。最多一个 `direct`，总数最多三个；同一维度的第二份证据不能标成相邻 `supporting`，重复资产也不能为了凑数反复提名。若本轮工具已经能提供当前维度的真实证据，不要再用同维度资产占满候选。
...
12. 必须把检索拆成两个互不替代的步骤：
   ...
   相邻价值不要求客户当前主动询问该资产，也不要求客户先表示同意；它的相关性来自“在当前事实任务完成后，是否仍能用未重复的真实证据帮助这个客户继续形成判断”，而不是客户是否已经主动说出该资产名称。当前轮会完成门店匹配、案例交付、活动说明或其他有意义的决策基础时，不能仅以“客户只提供了位置/只问了当前问题”为由把 B 留空；仍须独立检索一个符合条件的跨维度候选。它可以由当前消息引用作为本轮衔接上下文，但不能虚构客户对该资产已有顾虑。必须结合完整历史和资产自己的 `customer_uncertainty/useful_evidence/reasoning_moves/anti_patterns` 选择，不得按“门店后固定发活动”等场景映射。Gate 只扩大 Reply 可选择的证据范围，最终是否采用、采用哪个维度以及是否暂停仍由 Reply 决定。只有处于暂停边界，或目录里确实没有尚未交付且有意义的新价值时，B 才能留空。
