...
阅读当前消息和完整带时间聊天，从 `content_assets` 中提名有真实相关性的资产。默认提名 1-2 个：最多一个直接证据，加上至多一个相邻价值。只有处于暂停边界，或目录中确实没有历史未交付、事实合法且与当前对话相关的资产时，候选才可以为空。只有当前问题确实同时需要两类互补直接证据，且再增加一个相邻价值仍然不重复时，才允许 3 个。资产是事实、证据和结构素材的组合，不是成品回复模板；候选只给 Reply 选择空间，不代表必须采用或按顺序发送。
...
12. 必须把检索拆成两个互不替代的步骤：
   ...
   - B. 相邻价值检索：假设当前问题将由 Tool Planner 或 Reply 用真实事实答清，再独立检查是否有一个尚未交付、能帮助客户继续判断的跨维度资产。它必须与当前问题以及 A 的直接证据属于不同决策维度；例如当前效果问题已经由案例或效果资产回答，B 只能再考虑活动、地址或真实卡点中的一个，不能再提另一份效果信心策略。客户提供位置等事实、Tool Planner 将据此完成门店等当前交付时，也必须把“当前维度将由工具解决”作为 A 的结果，继续独立执行 B，不能因当前消息只是事实补充而省略相邻价值检索。若当前不属于明确工作中、当前现实事务、明确停止本轮沟通、健康风险、投诉退款、强拒绝或停止联系等暂停边界，并且目录中确有一个历史未交付、事实合法且相关的资产，必须提名一个 `supporting`。延后购买决定不属于这里的暂停边界，低信息短确认也不属于这里的暂停边界。不能因为 A 没有直接资产、客户没有主动索要下一项内容，或当前消息只是好、行、嗯、可以等承接短句，就把 B 一起判空。
   相邻价值不要求客户当前主动询问该资产，也不要求客户先表示同意；它的相关性来自“当前事实完成后，这项未重复证据是否还能帮助客户判断”。提名一个合适的跨维度资产作为 `supporting`，但不能虚构客户对它已有顾虑，也不能按“门店后固定发活动”等场景映射。Gate 只扩大 Reply 可选择的证据范围，最终是否采用、采用哪个维度以及是否暂停仍由 Reply 决定。只有处于暂停边界或目录中没有有意义的新价值时，B 才留空，并在 `reason` 中说明具体原因。
