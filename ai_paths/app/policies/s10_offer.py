    ...
    return [
        {
            **common,
            ...
            "conditions": "周年庆线上淡斑活动；仅限线上报名客户；限30名；套餐包括淡斑、检测皮肤、基础清洁、肌肤补水；当前登记预约赠送价值180元美白管理；线下客人未预定到店按原价1980元",
...
def attach_s10_offer_facts(fact_envelope: dict[str, Any] | None) -> dict[str, Any]:
    ...
    if isinstance(usable_facts, list) and not any("周年庆活动" in str(item) for item in usable_facts):
        usable_facts.append(
            "active_offer: S10周年庆线上淡斑活动；公开活动价268元；线上预约金10元，到店抵扣10元，做付258元；未做或不满意可退，实际按付款记录核对；预约金只锁活动名额，到店时间可按客户方便安排；限30名；当前登记预约赠送价值180元美白管理；集团连锁，全国300多家门店"
        )
    return envelope
