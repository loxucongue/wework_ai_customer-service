...
ACTIVE_S10_OFFER_CONTEXT: dict[str, Any] = {
    ...
    "brand_scale": "集团连锁，全国300多家门店；具体城市、地址、营业时间、停车和档期必须以 store_lookup / available_time 真实工具结果为准。",
    "method_summary": (
        "采用目前属于先进有效的第三代操作斑点技术，围绕淡斑、斑点和色素类改善方向承接。"
    ...
    "tail_payment": 258,
    "original_price": 1980,
    "original_price_visibility": "仅客户明确询问原价或线下未预定价格时回答1980元；主动活动介绍和压单只说名额满恢复原价，不主动报1980元。",
    "quota": 30,
    "package_items": ["淡斑", "检测皮肤", "基础清洁", "肌肤补水"],
    "campaign_package_name": "周年庆线上淡斑活动（淡斑套餐）",
    "signup_rule": "仅限线上报名客户有效；线上预定10元并留电话姓名，到店抵扣10元，做付258元；未做或不满意可退，实际按付款记录核对。预约金只锁活动名额，到店时间可按客户方便安排。",
    "walk_in_rule": "线下客人未预定到店按原价1980元；预定后名额到店做付258元。",
    "registration_skin_test": "完成线上活动登记后，可到线下门店免费做皮肤检测，由门店结合客户具体情况讲解；未登记时不得承诺免费检测。",
    "registration_gift": {
        "name": "美白管理",
        "stated_value": 180,
    ...
    "hard_close_benefit": "当前登记预约赠送价值180元的美白管理；不得扩展为其他赠品、现金抵扣、指定老师服务或永久有效。",
    "hard_constraints": [
        ...
        "客户问到店检测后价格怎么算、检测完会不会另算、方案出来多少钱时，直接说明当前线上活动套餐仍按268活动价：检测是为了看斑点和皮肤状态、确认适不适合及怎么操作，不是为了到店后重新把价格报成几百几千。",
        "老客价必须结合系统里的客户类型和上一次订单金额事实；没有订单金额事实时，只能说老客价需要按上次订单记录核对，不能对客户讲超过/不超过1000元对应的价格规则。",
        "10元预约金规则：到店抵扣10，做付258；未做或不满意可退，实际按付款记录核对；预约金只锁活动名额，不限制具体到店时间。",
        "10元预约金只是到店抵扣，不是额外优惠、省钱、减免或立减；不要说节省10元。",
        "不要把预约金说成不可退或已自动退款；客户可见时完整表达为到店抵扣10元、做付258元、未做或不满意可退，实际按付款记录核对。",
        "没有真实订单查询结果时，不要说正在查订单、稍等、稍后同步；只说老客价需要核对上次订单金额。",
        "“国内最先进、不伤皮肤、没有不良反应”只能作为销售原话参考，不得无条件原样承诺；客户可见时优先说先进、温和、到店检测后更准。",
        "压单只能使用当前活动事实：限30名、名额满恢复原价、10元预约金到店抵扣、到店时间后定、未做或不满意可退、当前登记预约赠送价值180元美白管理，以及已确认的主任/总监到店机会。主动活动介绍和压单不报原价金额；只有客户明确追问原价或线下未预定价格时才回答1980元。不得增加其他赠品、报销、接送、指定老师、固定日期或效果保证。",
        "效果信任轮按权威口径先说明我们是做斑点改善的、绝大多数顾客一次就有很好的改善效果；不得扩展成所有客户一次全部去除、根治、100%见效、永久不再出现、绝对安全或包效果。",
    ],
}
...
def s10_price_facts() -> list[dict[str, Any]]:
    ...
    common = {
        ...
        "suitable_for": "老年斑、遗传斑、各类斑点、黑色素、肤色不均等斑点和色素类改善方向",
    ...
    return [
        {
            **common,
            ...
            "display_price": "周年庆活动价268元，线上预约金10元，到店抵扣10元，做付258元；未做或不满意可退，实际按付款记录核对",
            "min_quote": str(context["new_customer_price"]),
            "conditions": "周年庆线上淡斑活动；仅限线上报名客户；限30名；套餐包括淡斑、检测皮肤、基础清洁、肌肤补水；完成线上活动登记后可到线下门店免费做皮肤检测并听取具体情况讲解；当前登记预约赠送价值180元美白管理；线下客人未预定到店按原价1980元",
...
def attach_s10_offer_facts(fact_envelope: dict[str, Any] | None) -> dict[str, Any]:
    ...
    if isinstance(usable_facts, list) and not any("周年庆活动" in str(item) for item in usable_facts):
        usable_facts.append(
            "active_offer: S10周年庆线上淡斑活动；公开活动价268元；线上预约金10元，到店抵扣10元，做付258元；未做或不满意可退，实际按付款记录核对；预约金只锁活动名额，到店时间可按客户方便安排；完成线上活动登记后可到线下门店免费做皮肤检测并听取具体情况讲解；限30名；当前登记预约赠送价值180元美白管理；集团连锁，全国300多家门店"
        )
    return envelope
...
- 可说明集团连锁、全国300多家门店；但具体城市、地址、营业时间、停车、档期必须以真实工具结果为准。
- S10 采用目前属于先进有效的第三代操作斑点技术，围绕淡斑、斑点和色素类改善方向承接；可提 ST 色素嫩肤从皮肤底层处理斑点问题，但不要讲成复杂科普。
- “国内最先进、不伤皮肤、没有不良反应”只作为销售原话参考，不要无条件原样承诺；客户可见优先改成“先进的斑点改善技术、主要围绕黑色素和斑点方向、整体更偏温和、到店检测后更准”。
- 适用方向：雀斑、晒斑、老年斑、遗传斑、痘印、痘坑、混合斑点、色素沉着等斑点和色素问题，以及客户明确咨询的痣类改善方向；这些范围先正向承接，再回案例、活动或门店主线。客户没有主动追问细分差异时，不在线追加凸起、大小、深浅等问题，也不能从错别字或含糊内容自行猜成痣。除皱、祛眼袋、黑眼圈和水光项目不属于当前线上可预约范围。
- 周年庆公开活动价 268 元；线上报名交 10 元预约金并留电话姓名，到店抵扣 10 元，做付 258 元；未做或不满意可退，实际按付款记录核对。预约金只锁活动名额，不限制具体到店时间。
- 客户问“测完皮肤后的价格怎么算/到店检测完是不是另算/方案出来多少钱”时，必须说明：当前线上活动套餐仍按 268 活动价，检测是为了看斑点和皮肤状态、确认适不适合和怎么操作，不是为了到店后重新报成几百几千。不要说“上来几百几千”“现场按方案再报价”“配置和服务影响价格”。
- 10 元预约金只是到店抵扣，不是额外优惠、省钱、减免或立减；不要说“节省10元”。
- 周年庆线上淡斑活动（淡斑套餐）包含：淡斑、检测皮肤、基础清洁、肌肤补水。
- 完成线上活动登记后，客户可以到线下门店免费做皮肤检测，由门店结合客户具体情况讲解；不能说未登记也可直接免费检测。
- 客户担心一次效果、可能需要多次、只是淡一点或是不是去斑时，本轮只解决效果信任：先明确说“我们是做斑点改善的，绝大多数顾客一次就有很好的改善效果”，再说明完成线上活动登记后可到门店免费做皮肤检测并用真实效果案例建立信任；本轮不得报价、复述活动规则或发送预约金卡，也不得主动补“可能需要多次、具体看深浅和时间”等削弱信心的话。不得扩展为所有客户一次全部去除、根治或永久不再出现。
- 活动限 30 名；主动活动介绍和压单只说名额满恢复原价，不主动报原价金额。只有客户明确追问原价或线下未预定价格时，才说明原价 1980 元；预定后名额到店做付 258 元。
- 当前登记预约赠送价值 180 元的美白管理。只允许按该活动事实表达，不得扩展为其他赠品、现金抵扣、指定老师服务或永久有效。
...
- 预约金口径固定说“到店抵扣10元，做付258元；未做或不满意可退，实际按付款记录核对；到店时间按客户方便安排”；老客价没有真实订单查询结果时，不要说“我去查订单、稍等、稍后同步”。
- 压单每轮只使用一个当前活动事实或本轮权威事实。不得使用旧版小气泡赠送口径，也不得新增赠品、报销、接送、指定老师、固定日期或效果保证。
""".strip()
