...
class PlatformAgentClient:
    ...
    def __init__(self, settings: Settings) -> None:
        ...
        self._test_mode_enabled = bool(settings.platform_agent_test_mode_enabled)
        self._test_force_user_id = settings.platform_agent_test_force_user_id
        self._test_force_corp_id = settings.platform_agent_test_force_corp_id
        self._test_force_wechat = settings.platform_agent_test_force_wechat

    @property
    def available(self) -> bool:
        return bool(self._base_url and self._token)

    def effective_identity(self, request_context: dict[str, Any] | None = None) -> dict[str, str]:
        merged = self._with_common_params({}, request_context)
        return {
            "user_id": str(merged.get("user_id") or "").strip(),
            "corp_id": str(merged.get("corp_id") or "").strip(),
            "wechat": str(merged.get("wechat") or "").strip(),
        }
    ...
    def get_customer_info(
        ...
        *,
        ...
        if not external_userid:
            return {}
        params = self._with_identity_params(
            {
                "user_id": user_id,
                "corp_id": corp_id,
                "wechat": wechat,
    ...
    def change_order_plan_time(
        ...
        *,
        ...
        data = self._post(
            ...
            self._with_common_params(payload, request_context),
        )
        return data if isinstance(data, dict) else {"result": data}
    ...
    def _with_common_params(self, params: dict[str, Any], request_context: dict[str, Any] | None) -> dict[str, Any]:
        ...
        if self._test_mode_enabled:
            return self._with_identity_params(merged)
    ...
    def _with_identity_params(self, params: dict[str, Any]) -> dict[str, Any]:
        merged = dict(params)
        if self._test_mode_enabled:
            if self._test_force_user_id not in (None, ""):
                merged["user_id"] = self._test_force_user_id
            elif merged.get("user_id") in (None, "") and self._default_user_id not in (None, ""):
                merged["user_id"] = self._default_user_id
            if self._test_force_corp_id:
                merged["corp_id"] = self._test_force_corp_id
            elif merged.get("corp_id") in (None, "") and self._default_corp_id:
                merged["corp_id"] = self._default_corp_id
            if self._test_force_wechat:
                merged["wechat"] = self._test_force_wechat
            elif merged.get("wechat") in (None, "") and self._default_wechat:
                merged["wechat"] = self._default_wechat
            return merged
        if merged.get("user_id") in (None, "") and self._default_user_id not in (None, ""):
            merged["user_id"] = self._default_user_id
