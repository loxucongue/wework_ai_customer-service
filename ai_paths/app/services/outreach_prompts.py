...
6. 只有报价已完成时，最后一步才可使用 `transaction` 发卡；一旦发卡，文字和 CTA 都直接引导点击随消息附上的 10 元预约金卡。
...
- 已有完整报价、客户只是价格/效果/距离等普通顾虑且前两轮已完成化解时，最后一轮优先直接附预约金卡；文字直接说明 10 元保留活动资格，不要再让客户回复“活动/入口”。
...
- 这类客户已有完整活动报价且最后一轮选择附卡时，`draft_text` 和 `cta` 都必须明确“我把10元预约金卡发您，点开支付即可保留资格”；不能仍停留在“给您登记一个吗/要不要我发卡”。
...
7. `source_snapshot.activity_quote_fact.completed=true` 且最终决定发卡时，只能在最后一步发一次；文字必须表达“我把10元预约金卡发您”并直接引导使用，不能说“要的话再发”“回复入口后再发”或翻旧卡。
