...
class SopReplyPackService:
    ...
    def load(self) -> dict[str, Any]:
        if not self.path.exists():
            payload = deepcopy(DEFAULT_SOP_REPLY_PACKS)
            normalized = self._normalize(
                self._apply_overlay(payload),
                allow_legacy_event_scopes=True,
            )
        ...
        try:
            ...
            normalized = self._normalize(
                self._apply_overlay(deepcopy(DEFAULT_SOP_REPLY_PACKS)),
                allow_legacy_event_scopes=True,
            )
            normalized["audit"] = _audit_config(normalized)
            return normalized
    ...
    def _apply_overlay(self, payload: dict[str, Any]) -> dict[str, Any]:
        """Merge code-owned V2 metadata without copying customer-visible content."""
        ...
        for index, item in enumerate(raw_overlay_packs):
            ...
            if not pack_id:
                raise ValueError(f"SOP overlay pack #{index + 1} id is required")
            if pack_id in seen:
                raise ValueError(f"duplicated SOP overlay pack id: {pack_id}")
            seen.add(pack_id)
            ...
            if illegal:
                raise ValueError(
                    f"SOP overlay pack {pack_id} contains business-content fields: {','.join(illegal)}"
                )
