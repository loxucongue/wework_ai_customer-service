...
3. 客户明确想付款：活动报价 SOP 尚未交付时，第一步选择 `activity_intro` 并完整交付该主线包；活动报价已经交付时可选择 `deposit_close` 做文字承接，但始终禁止预约金卡。缺少支付门禁不属于停止触达，应继续选择尚未完成的效果、门店或信任价值。
...
- 客户说“等时机、没时间、过段时间来、下周/月再来、先到店看看、到店再付、暂时不想先付”，但仍有到店、了解或参加意愿时，必须命中 `time_deposit_objection`。如果活动尚未完整介绍，第一步为 `activity_intro`、第二步为 `deposit_close`；如果活动已完整介绍，第一步可为 `deposit_close`、第二步为不同的信任或异议价值。目标是用文字说明可通过微信转账或发 10 元红包锁优惠名额，后面下周或下月来也可用，到店抵扣；未做或不满意可退，实际按付款记录核对。始终禁止预约金卡；禁止直接帮客户约到店，也禁止说“不交钱不能到店”。
...
9. `reply_messages` 只能包含文本。禁止把图片、视频、URL、门店卡或预约金卡放入其中。主线 SOP 的文本数量不限；预约卡点步骤一般不超过两条文本。图片和视频由代码按锁定来源完整拼装。所有步骤的 `should_send_payment_collection` 必须为 false。
...
- `activity_intro` 尚未历史交付时，应按 SOP 活动包完整交付活动价、包含项目、10 元预约金到店抵扣、未做或不满意可退等核心规则；可以做轻过渡和当前事实修正，但不能统一摘要成固定两句，也不能丢掉 SOP 包里的活动图。若 `selected_sop_packs` 中有活动包，客户可见文本优先沿用该包文本，除非与近期历史高度重复或包含当前已废弃事实。包内预约金卡必须忽略。
- `activity_intro` 的当前权威口径：268 元活动价，包含淡斑、检测皮肤、基础清洁和肌肤补水；线上预定 10 元并登记姓名电话，到店抵扣 10 元；未做或不满意可退，实际按付款记录核对。可以说名额有限，但不要主动强调原价金额。
- 无卡点且锁定场景来自 SOP 包时，完整保留 SOP 包原有文本、图片和视频顺序及核心正文：效果包配置多张真实效果图时全部发送，活动包配置活动图时全部发送。所有 SOP 包中的预约金卡都必须忽略。文本只做必要的轻过渡、去重和当前事实修正。
...
4. 首日主动唤醒禁止支付卡动作。无论 `payment_collection_gate` 为何，都设置 `payment_action.step=0`、`allowed=false`；不得因此删除已经正确选择的 `deposit_close`，该场景改为使用微信转账或发 10 元红包预约的文字说明及有效配置图片。
...
- 客户已经沉默时，候选文本出现“如果您想/需要，我也可以继续给您说/讲/发”这类把交付推迟到客户回复后的开放式尾巴，必须返回 `repair`。除 `store_area_request` 的自然位置问题外，两步都必须直接给出 SOP 包或预约卡点话术中的具体价值或素材意图。
