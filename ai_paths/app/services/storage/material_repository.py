...
class MaterialRepositoryMixin:
    ...
    def apply_material_catalog(
        ...
        before = []
        after = []
        lock = " FOR UPDATE" if self.store.dialect == "mysql" else ""
        with self.store.connect() as conn:
            for index, row in enumerate(rows):
                ...
                old = conn.execute(
                    "SELECT * FROM material_identities WHERE alias_key=?" + lock, (row["alias_key"],)
                ).fetchone()
                ...
                if old and old["canonical_id"] != row["canonical_id"]:
                    ...
                    claims = conn.execute(
                        "SELECT 1 FROM material_claims WHERE alias_key=? LIMIT 1" + lock, (row["alias_key"],)
                    ).fetchone()
                    if claims:
    ...
    def rollback_material_catalog(self, undo: dict[str, Any]) -> None:
        ...
        with self.store.connect() as conn:
            for expected, prior in zip(reversed(undo["after"]), reversed(undo["before"])):
                ...
                row = conn.execute(
                    "SELECT * FROM material_identities WHERE alias_key=?" + lock, (expected["alias_key"],)
                ).fetchone()
    ...
    def claim_materials_in_connection(
        ...
        lock = " FOR UPDATE" if self.store.dialect == "mysql" else ""
        seen = set()
        # Acquire claim keys in the same order even if Reply orders media
        # differently. Stable client-message IDs and output order do not change.
        for message in sorted(
            messages,
            key=lambda item: str(
                (bindings.get(f"{item.get('type')}:{media_url(item)}") or {}).get("canonical_id") or ""
            ),
        ):
            ...
            identity = conn.execute(
                "SELECT canonical_id FROM material_identities WHERE alias_key=?" + lock, (binding["alias_key"],)
            ).fetchone()
            ...
            # INSERT IGNORE waits for a competing transaction, but a plain
            # SELECT under REPEATABLE READ can still see its earlier snapshot.
            # The locking read above sees the committed winner instead.
            if (
                not row
                or row["request_id"] != request_id
                or row["client_message_id"] != str(message.get("client_message_id") or "")
            ):
