...
internal_work_items = Table(
    f"{TABLE_PREFIX}internal_work_items",
    metadata,
    _id("id", primary_key=True),
    _id("idempotency_key"),
    _short("work_type", length=64),
    _short("status", "pending", length=32),
    _id("store_id"),
    _short("detail_kind", length=64),
    _json("missing_fact_keys_json", "[]"),
    _id("source_request_id"),
    _id("conversation_id"),
    _id("corp_id"),
    _id("wechat"),
    ...
    _time("resolved_at", ""),
    _id("resolved_by"),
    _long("resolution_note"),
    _json("payload_json", "{}"),
    _time("created_at"),
    _time("updated_at"),
    UniqueConstraint("idempotency_key", name="uq_aics_internal_work_items_idempotency"),
    Index("idx_aics_internal_work_items_queue", "work_type", "status", "last_seen_at"),
    Index("idx_aics_internal_work_items_store_detail", "store_id", "detail_kind", "status"),
    Index(
...
v3_strategy_usage_events = Table(
    ...
    Column("sequence_adopted", Integer, nullable=False, server_default="0"),
    Column("script_adopted", Integer, nullable=False, server_default="0"),
    Column("adoption_detail_observed", Integer, nullable=False, server_default="0"),
    _id("dispatch_id"),
