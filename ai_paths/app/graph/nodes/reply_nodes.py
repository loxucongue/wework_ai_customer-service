...
def _reply_repair_hint(error: str) -> str:
    ...
    if (
        "store_cards_not_allowed_for_text_store_list" in error
        or "invalid_text_store_list_contract" in error
        or "incomplete_text_store_list_contract" in error
    ):
        return (
    ...
    if "available_time_fact_required" in error:
        return (
            "没有 available_time slots 时，不要声称具体时段有空档、有空位、有名额或档期已经确认。"
            "可以保留‘可以预约、时间可协调’，客户已给出时间时可以说‘这个时间可以协调、先作为到店意向’，"
            "但不能改写成已预约、已安排、已登记、已留位或可以直接到店。"
