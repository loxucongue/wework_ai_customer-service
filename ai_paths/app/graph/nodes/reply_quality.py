...
def _has_unbacked_distance_claim(state: AgentState, text: str) -> bool:
    if not _DISTANCE_CLAIM_PATTERN.search(text):
        ...
        if not any(term in query for term in ("哪家近", "离我近", "近一点", "最近", "附近哪家")):
            return False
        if not (any(term in text for term in ("推荐", "优先", "最近", "近一点", "离您近")) or _STORE_CONCRETE_PATTERN.search(text)):
            return False
