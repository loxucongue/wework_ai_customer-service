...
def postprocess_reply_messages(
    ...
    result = _rewrite_passive_opening_reply(state, result)
    result = _rewrite_bare_greeting_reply(state, result)
    result = _rewrite_low_info_business_reply(state, result)
    result = _ensure_signup_progression(state, result)
    result = _sanitize_unverified_signup_store_reference(state, result)
    result = _rewrite_visit_arrangement_bare_reply(state, result)
    result = _rewrite_too_far_overpush_reply(state, result)
    result = _ensure_store_address_answer(state, result)
    result = _rewrite_store_confirmation_burden(result)
    result = _rewrite_opening_city_collection(state, result)
    result = _merge_compact_text_messages(result)
    result = _ensure_case_followup_question(state, result, ask_policy=ask_policy)
    result = _ensure_age_suitability_next_step(state, result)
    result = _rewrite_city_only_store_prompt(state, result)
    result = _normalize_need_intro_wording(state, result)
    result = _rewrite_type_followup_reply(state, result)
    result = _compact_trailing_question(state, result, ask_policy=ask_policy)
    result = _ensure_ad_multi_intent_answer(state, result)
    result = _strip_case_times_extra_followup(state, result)
    result = _ensure_sales_hook_after_effect_or_no_price(state, result)
    result = _ensure_key_business_answer(state, result)
    result = _ensure_deposit_refusal_answer(state, result)
    result = _sanitize_unverified_store_time_hook(state, result)
