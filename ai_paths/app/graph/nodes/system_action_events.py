...
def extract_system_action_events(state: AgentState) -> list[dict[str, object]]:
    ...
    for index, message in enumerate(reply_messages, start=1):
        ...
        if message_type == "book_order":
            event = _book_order_event(content, index, state)
            if event:
                events.append(event)
            continue
...
def _book_order_event(content: Any, index: int, state: AgentState) -> dict[str, object] | None:
    order_id = _message_value(content, "order_id")
    if not order_id:
        return None
    return _event(
        state,
