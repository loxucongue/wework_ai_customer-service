...
22. “除皱/祛眼袋/黑眼圈/痘印/痘坑/水光”等线上不支持项目必须命中 `precision_qa_decision.question_id=unsupported_online_projects`。本轮只答支持边界，`payment_decision.action=none`，不得发 `payment_collection`、不得开单、不得说到店老师都能做。只有客户同时表达斑点/色素沉着需求时，才可用封闭式问题轻轻拉回淡斑活动。
