...
def has_store_inquiry(content: str) -> bool:
    ...
    trust_terms = ["正规", "靠谱吗", "骗人", "被骗", "资质", "营业执照", "证照", "许可证", "真假", "隐形消费", "被坑", "安全", "售后"]
    ...
    if not any(term in content for term in ["店", "门店"]):
        return False

    appointment_terms = ["预约", "能约", "能去", "周六", "周日", "明天", "后天", "下午", "上午", "到店"]
    if any(term in content for term in appointment_terms):
        return False
    return True
