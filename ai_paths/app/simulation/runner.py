...
def render_markdown(report: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        "## 版本证据",
        "",
        f"- Git commit：{report.get('git_commit') or '缺失'}",
        f"- Git commit set：{', '.join(str(item) for item in report.get('git_commit_set') or []) or '缺失'}",
        f"- Fixture：{report.get('fixture') or '缺失'}",
        "",
        "## 运行范围与选项",
        "",
