...
async def run(args: argparse.Namespace) -> None:
    ...
    async def one(case: dict[str, Any]) -> None:
        ...
        if args.register_synthetic_media:
            records = []
            for pack in SopReplyPackService(runtime["settings"]).load().get("packs") or []:
                for message in pack.get("reply_messages") or []:
                    ...
                    if url:
                        records.append({
                            "type": message["type"], "url": url, "file_id": len(records) + 1,
                            "file_namespace": "follow_knowledge", "source": "synthetic_l3_fixture",
                        })
