...
async def _main() -> int:
    parser = argparse.ArgumentParser(description="Run the V3 DeepSeek destination resolver address matrix once.")
    ...
    if args.enable_fallback:
        raise RuntimeError("DeepSeek-only address tests do not permit cross-model fallback")
    assert_deepseek_models(args.router_model)
    settings = deepseek_only_settings(
        Settings(), router_model=args.router_model, reply_model="deepseek-v4-pro"
    ).model_copy(
        update={
