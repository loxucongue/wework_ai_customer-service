...
def _knowledge_candidates(result: dict[str, Any]) -> set[str]:
...
def evaluate(golden: dict[str, Any], results: dict[str, Any]) -> dict[str, Any]:
    ...
    for case_id, case in cases.items():
        ...
        if result is None:
            rows.append(row)
            continue
        nominated = _knowledge_candidates(result)
        ...
        row.update(
            {
                "knowledge_recall": 1.0 if not required_gate else len(required_gate & nominated) / len(required_gate),
    ...
    summary["gate_recall"] = summary.get("knowledge_recall")
    first_inquiry_cases = [
        row
