...
def _model_usage(state: dict[str, Any]) -> list[dict[str, Any]]:
    # Model calls live under trace[*].tool_calls (and may be nested again for
    # repair/recovery). Keep only the provenance names in this matrix report;
    # the full trace remains the source for detailed token/call diagnostics.
    return [
        {"model": model}
        for model in collect_model_names(state.get("trace") or [])
    ]
...
async def _run_case(
    *,
    ...
    return {
        ...
        "model_usage": _model_usage(final_state),
    }
...
async def _main() -> int:
    ...
    actual_models = collect_model_names([item.get("model_usage") for item in cases])
    report["actual_models"] = actual_models
    report["model_audit"] = {
        "router_model": args.router_model,
        "router_recorded": args.router_model in actual_models,
        "reply_model": args.reply_model,
        "reply_recorded": args.reply_model in actual_models,
    }
    ...
    # Preserve the expensive raw run before failing model-provenance checks.
