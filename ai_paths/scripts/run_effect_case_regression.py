...
REPORT_FILE = ROOT / "logs" / "effect_case_regression_report.json"
...
def _reply_text(item: dict[str, Any]) -> str:
...
def _assistant_messages(result: dict[str, Any]) -> list[str]:
    ...
    if not isinstance(messages, list):
        return []
    return [_reply_text(item) for item in messages if isinstance(item, dict) and _reply_text(item)]
...
def run() -> list[dict[str, Any]]:
    ...
    customer_id = f"codex_effect_case_regression_{int(time.time())}"
    history: list[str] = []
        ...
        for msg_index, reply in enumerate(replies, start=1):
            ...
            if msg_index == 1:
                message["meta"] = {
                    ...
                    "toolResultKeys": tool_result_keys,
        ...
        report.append(
            {
                ...
                "case_studies_hit": "case_studies" in tool_result_keys,
                "reply": replies,
