...
def test_prompts_do_not_restore_scene_matching_or_raw_reference_replies() -> None:
    ...
    assert "跟进序列说明为什么采取某个动作" in PARALLEL_REPLY_SYSTEM_PROMPT
    assert "其他节点只提供事实或候选证据，不能替你作销售决定" in PARALLEL_REPLY_SYSTEM_PROMPT
    assert "第一次介绍活动或价格只讲活动和价值" in PARALLEL_REPLY_SYSTEM_PROMPT
    assert "活动和预约金分开" in PARALLEL_REPLY_SYSTEM_PROMPT
