...
def test_text_only_primary_script_can_be_recorded_without_forcing_asset_delivery() -> None:
    metadata = _reply_metadata_from_model_call(
        _model_call(
    ...
    assert metadata["selected_content_ids"] == []
    assert metadata["knowledge_use"]["selected_script_ids"] == ["D02"]
    assert metadata["knowledge_use"]["sequence_id"] == "seq-distance-1"


def test_unknown_primary_script_id_is_not_recorded() -> None:
    metadata = _reply_metadata_from_model_call(
        _model_call(selected_content_ids=[], knowledge_use={"script_id": "invented-script"}),
        state=_state(),
    )
...
def test_script_code_selection_is_normalized_to_platform_script_id() -> None:
    state = _state()
    state["sales_recall"]["candidates"] = [
        {
            "script_id": "116",
            "source_id": "KD-0116",
            "id": 116,
    ...
    metadata = _reply_metadata_from_model_call(
        _model_call(
            ...
            knowledge_use={"sequence_id": "seq-distance-1", "step_id": "step-case"},
        ),
        state=state,
    )

    assert metadata["knowledge_use"]["selected_script_ids"] == ["116"]


def test_content_reference_in_knowledge_use_is_normalized_to_platform_script_id() -> None:
    state = _state()
    state["sales_recall"]["candidates"] = [
        {
            "script_id": "116",
    ...
    metadata = _reply_metadata_from_model_call(
        _model_call(
            selected_content_ids=[],
            knowledge_use={"script_id": "follow_script:KD-0116:p1"},
        ),
        state=state,
    )
