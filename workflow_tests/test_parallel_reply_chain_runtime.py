...
from app.prompts.reply_synthesizer import (
    ...
    _render_execution_capabilities,
    _render_payment_channel_availability,
    _render_protocol_events,
...
from app.services.v3_sop_execution_service import (
    SopExecutionService,
...
def test_completed_activity_asset_is_visible_as_prior_delivery_fact() -> None:
    observation = _asset_delivery_observation(
        {"content_id": "s10_activity_intro", "asset_role": "activity_offer"},
        {"activity_intro_image_sent": False},
        sop_progress={"completed_pack_ids": ["s10_activity_intro"]},
    )

    assert observation["sent_count"] == 1
    assert observation["source"] == "sop_progress_or_sent_message_summary"
        ...
        [
            {
                ...
                "messages": [
                    {"type": "image", "content": {"url": "https://example.com/activity.png"}}
                ],
            }
        ],
        sent_summary={"activity_intro_image_sent": True},
        sop_progress={},
    )

    assert len(assets) == 1
    assert assets[0]["delivery_status"] == "completed"
    assert assets[0]["delivery_observation"]["sent_count"] == 1


def test_selected_content_media_id_placeholder_resolves_to_candidate_url() -> None:
    state = _parallel_state("一次能好吗")
    state["evidence_join"]["content_candidates"] = [
        {
            ...
            "messages": [
                {
                    "type": "image",
                    "content": {"url": "https://example.com/effect-1.png"},
                },
                {
                    "type": "image",
                    "content": {"url": "https://example.com/effect-2.png"},
                },
            ],
        }
    ]
    payload = {
        ...
        "reply_messages": [
            {"type": "text", "content": "我把真实效果参考发您看下。"},
            {"type": "image", "content": "s10_need_and_case"},
        ],
    }

    assert _resolve_selected_content_media_placeholders(payload, state) is True
    assert payload["reply_messages"][1]["content"] == "https://example.com/effect-1.png"
    _validate_parallel_raw_reply_schema(payload)


def test_completed_content_id_is_not_selectable_this_turn() -> None:
    state = _parallel_state("有空再说")
    state["evidence_join"]["content_candidates"] = [
        {
            "content_id": "s10_activity_intro",
            "delivery_status": "completed",
            "messages": [
                {"type": "image", "content": "https://example.com/activity.png"},
            ],
        }
    ]
    ...
    with pytest.raises(
        ValueError,
        match="selected_content_id_not_selectable:s10_activity_intro",
    ):
        _validate_selected_content_ids(
            {"selected_content_ids": ["s10_activity_intro"]},
            state,
        )
...
def test_v3_deposit_asset_does_not_force_the_one_person_card_template() -> None:
        ...
        [
            {
                ...
                "messages": [
                    {"type": "text", "content": "每位10元预约金。"},
                    {"type": "image", "content": "https://example.com/deposit.png"},
    ...
    assert [item["type"] for item in assets[0]["messages"]] == ["text", "image"]
...
def test_parallel_gate_only_nominates_content_assets() -> None:
    ...
    assert "不输出客户话术、销售动作、工具调用或写操作" in prompt
    assert "direct" in prompt
    assert "supporting" in prompt
    assert "不按关键词、场景 ID 或配置顺序匹配" in prompt
...
def test_parallel_content_gate_does_not_use_opening_asset_for_substantive_questions() -> None:
    assert "依据当前消息、完整聊天、资产用途和真实交付记录判断相关性" in PARALLEL_CONTENT_GATE_SYSTEM_PROMPT
    assert "先找能直接证明当前问题的 `direct` 资产" in PARALLEL_CONTENT_GATE_SYSTEM_PROMPT
...
def test_parallel_reply_prompt_uses_history_without_fixed_short_ack_script() -> None:
    ...
    assert "客户明确说正在工作、开车或此刻无法交流" in prompt
    assert "泛泛的考虑一下、晚点说、有空看看不是明确停止" in prompt
    assert "阻力已知时，换一个尚未重复" in prompt
...
def test_protocol_event_summary_preserves_unverified_red_packet_boundary() -> None:
    ...
    assert events == [
        {
            "event_type": "external_redpacket",
            "direction": "customer_to_staff",
            "delivery_status": "received_unverified",
            "payment_channel": "red_packet",
            "payment_status": "unknown",
            "source": "merged_customer_messages:2",
        }
    ]
    rendered = _render_protocol_events(events)
    assert "客户本轮已发送红包类平台消息" in rendered
    assert "不重复发送预约金图、付款卡或其他付款入口" in rendered
    assert "支付状态=unknown" in rendered


def test_protocol_payment_event_removes_duplicate_payment_delivery_options() -> None:
    state = _parallel_state("[external_redpacket消息]")
    ...
    assert availability["red_packet"]["allowed"] is False
    assert "不得再次要求客户发红包" in _render_payment_channel_availability(availability)


def test_activity_fact_distinguishes_offer_from_package_selection() -> None:
    rendered = _render_authoritative_facts(
        {
            "AUTHORITATIVE FACTS": {
                "offer": {
                    "public_names": ["周年庆淡斑活动"],
                    "new_customer_price": 268,
                    "body_scope": "单部位体验",
                    "offer_structure": "268元对应当前周年庆淡斑活动项目，不是到店后再从多个套餐中选择。",
                    "includes": ["淡斑", "基础清洁"],
                }
            }
        },
    ...
    assert "不是到店后再从多个套餐中选择" in rendered
    assert "淡斑、基础清洁" in rendered


def test_parallel_business_rules_preserve_offer_structure_fact() -> None:
    rules = parallel_reply_business_rules_for_model()

    assert "不是到店后再从多个套餐中选择" in rules["AUTHORITATIVE FACTS"]["offer"][
        "offer_structure"
    ]


def test_store_conclusion_preserves_local_absence_and_city_fallback() -> None:
    rendered = _render_store_resolution_conclusion(
        {
            "status": "send_single",
            "exact_scope_has_store": False,
            "same_city_has_store": True,
            "scope_match_level": "city_fallback",
            "delivery_store_ids": ["383"],
        }
    )

    assert "具体区县/乡镇本地没有门店" in rendered
    assert "同一城市其他区域" in rendered
    assert "不得说成客户所述地点本地有店" in rendered
    assert "门店ID=383" in rendered


def test_reused_store_conclusion_preserves_scope_without_duplicate_card() -> None:
    rendered = _render_store_resolution_conclusion(
        {
    ...
    assert "具体区县/乡镇本地没有门店" in rendered
    assert "不得重复发送 store_address" in rendered
    assert "此前门店ID=383" in rendered
    assert constraints[0]["code"] == "store_result_already_delivered"
    assert "不能改变其结论" in constraints[0]["instruction"]


def test_reply_prompt_requires_direct_asset_relevance_and_no_redundant_narrowing() -> None:
    assert "不要把它拆成更窄的二选一再问一遍" in PARALLEL_REPLY_SYSTEM_PROMPT
    assert "直接支持本轮主要目标或唯一一个相邻新价值" in PARALLEL_REPLY_SYSTEM_PROMPT
    assert "已经选择并会随本轮交付的素材" in PARALLEL_REPLY_SYSTEM_PROMPT
    assert "没有合法 commit_action 时只能说已经收到" in _render_execution_capabilities()
    assert "不能先说“对、流程对、您理解得对”" in PARALLEL_REPLY_SYSTEM_PROMPT


def test_parallel_content_gate_does_not_reopen_completed_fact_collection_from_progress_gaps() -> None:
    prompt = PARALLEL_CONTENT_GATE_SYSTEM_PROMPT

    assert "已交付资产默认不重发" in prompt
    assert "客户明确要求重发、更多、新证据，或提供冲突事实" in prompt


def test_parallel_content_gate_distinguishes_delayed_decision_from_unavailable_conversation() -> None:
    prompt = PARALLEL_CONTENT_GATE_SYSTEM_PROMPT

    assert "不决定推进、暂停、成交或下一步" in prompt
    assert "Reply 可采用、组合或忽略全部候选" in prompt


def test_parallel_content_gate_treats_recent_real_delivery_as_completed_and_still_searches_adjacent_value() -> None:
    prompt = PARALLEL_CONTENT_GATE_SYSTEM_PROMPT

    assert "真实交付记录" in prompt
    assert "已交付资产默认不重发" in prompt
    assert "仍可独立提名一个跨维度 supporting 资产" in prompt
...
def test_parallel_reply_rule_view_keeps_unique_sections_without_repeating_layered_facts() -> None:
    ...
    assert set(rules) == {"MUST FOLLOW", "AUTHORITATIVE FACTS", "FACT TOPIC CATALOG"}
    assert len(rules["FACT TOPIC CATALOG"]) == 14

    assert rules["AUTHORITATIVE FACTS"]["offer"]["new_customer_price"] == 268
    assert "reservation_completion_rule" not in rules["AUTHORITATIVE FACTS"]["offer"]
    assert "appointment_flow_mode" not in rules["AUTHORITATIVE FACTS"]["transaction_policy"]
    ...
    assert "TOOL FACT BOUNDARIES" not in rules
...
def test_parallel_reply_treats_immediately_prior_complete_offer_as_prior_evidence() -> None:
    ...
    assert "地址、效果或卡点中的一项真实承接" in PARALLEL_REPLY_SYSTEM_PROMPT
...
def test_parallel_reply_payload_excludes_completed_asset_from_selectable_ids() -> None:
    state = {
        "evidence_join": {
            "shared_context": {"conversation": []},
            "content_candidates": [
    ...
    assert payload["allowed_selected_content_ids"] == ["s10_need_and_case"]
    assert payload["content_candidate_reference_options"] == [
        {
            "content_id": "s10_need_and_case",
            "used_fact_ref": "content_asset:s10_need_and_case",
        }
    ]
...
def test_parallel_reply_payload_surfaces_current_store_delivery_as_an_option() -> None:
    ...
    assert payload["store_fact_status"] == {
        ...
        "delivery_store_ids": [],
        "already_delivered_store_ids": [],
        "candidate_search_complete": False,
        "resolved_admin_level": "",
        "scope_match_level": "",
        "exact_scope_has_store": None,
        "same_city_has_store": None,
...
def test_parallel_reply_payload_keeps_non_delivery_store_fact_without_empty_delivery_contract() -> None:
    ...
    assert "store_address" not in payload["structured_delivery_options"]
    assert [
        item["content"]["amount"]
        for item in payload["structured_delivery_options"]["payment_collection"]["message_payloads"]
    ] == [10, 20, 30, 40]
    assert payload["tool_fact_reference_options"] == [
        {
            "ref": "tool_fact:customer_store_lookup",
...
def test_parallel_reply_payload_surfaces_payment_options_without_python_sales_stage() -> None:
    ...
    assert [item["content"]["amount"] for item in option["message_payloads"]] == [10, 20, 30, 40]
    assert "reply_must_find_current_customer_payment_action_signal" in option["constraints"]
...
def test_parallel_reply_payload_hides_payment_options_for_authoritative_paid_customer() -> None:
    state = _parallel_state("我已经付过了")
    state["payment_state"] = "deposit_paid"
    state["evidence_join"] = {
        "shared_context": {
            "conversation": [],
            "current_message": {"content": "我已经付过了"},
        },
        "content_candidates": [],
        "tool_facts": {},
    }
...
def test_parallel_reply_prompt_treats_delivered_key_as_foundation_without_customer_approval() -> None:
    assert "真实素材能直接降低当前疑虑" in PARALLEL_REPLY_SYSTEM_PROMPT
...
def test_parallel_reply_payload_summarizes_paid_registration_field_presence() -> None:
    ...
    assert payload["registration_fact_status"] == {
        ...
        "missing_fields": ["customer_mobile", "arrival_intent"],
        "source": "authoritative_facts.registration_facts",
    }


def test_parallel_reply_payload_exposes_payment_channels_and_confirmed_registration_store() -> None:
    state = _parallel_state("姓名电话发您了")
    shared_context = {
        "current_message": {"content": "姓名电话发您了"},
        "conversation": [],
        "authoritative_facts": {
            "orders_and_payment": {
            ...
            "request_store_facts": {
                "confirmed_store_id": "601",
                "confirmed_store_name": "荆州区店",
            },
        },
    }
    state["shared_context"] = shared_context
    ...
    assert payload["payment_channel_availability"] == {
        "payment_card": {"available": False},
        "transfer": {"allowed": True, "qr_code_available": False},
        "red_packet": {"allowed": True},
        "account_or_qr_facts": [],
        "source": "platform_payment_channel_contract",
    }
    assert payload["registration_fact_status"]["confirmed_store"] == {
        "store_id": "601",
        "store_name": "荆州区店",
    }
    assert payload["registration_fact_status"]["missing_fields"] == ["arrival_intent"]
...
def test_parallel_reply_payload_prefers_canonical_store_resolution_over_raw_intermediate() -> None:
    state = {
        "fact_envelope": {
            "structured_facts": {
                "store_resolution_fact": {
        ...
        "evidence_join": {
            "shared_context": {"conversation": [], "current_message": {"content": "乌林村"}},
            "tool_facts": {
                "customer_store_lookup": {
                    "store_resolution_fact": {
                        "status": "send_single",
                        "delivery_store_ids": ["701"],
                    }
                }
            },
            "normalized_tool_facts": {
    ...
    option = parallel_reply_payload(state)["structured_delivery_options"]["store_address"]

    assert option["available_store_ids"] == ["243"]
...
def test_parallel_reply_repair_rechecks_root_action_before_fixing_structure() -> None:
    ...
    assert "先核对上一版客户可见动作是否被本轮真实引用和硬事实允许" in instruction
    assert "若动作本身不合法，必须撤销冲突的结构消息" in instruction
    assert "不要因为错误写着‘缺卡片’就直接加卡" in instruction
    assert "每个 selected_content_ids 都必须来自允许候选" in instruction
    assert "本轮实际发送 payment_collection" in instruction
    assert "权威已付登记不得再次发送 payment_collection" in instruction
    assert "used_fact_refs" not in instruction
...
def test_parallel_structural_repair_guard_allows_full_asset_delivery_or_deselection() -> None:
    ...
    assert "不交付整套素材时，删除该 ID" in guard
    assert "used_fact_refs" not in guard
...
def test_parallel_reply_health_policy_distinguishes_assessment_from_operation() -> None:
    assert "健康风险" in PARALLEL_REPLY_SYSTEM_PROMPT
    assert "不发卡" in PARALLEL_REPLY_SYSTEM_PROMPT
    assert "健康风险" in PARALLEL_REPLY_SYSTEM_PROMPT
...
def test_parallel_reply_schema_derives_payment_action_from_structured_message() -> None:
    payload = {
        "reply_messages": [
            {"type": "text", "content": "预约金规则给您发过来。"},
            {"type": "payment_collection", "content": {"amount": 10, "remark": ""}},
        ],
        "sales_judgment": {
            "customer_friction_observation": "",
            "primary_objective": "完成预约金支付",
            "posture": "close",
        },
        "payment_assessment": {
            "status": "payment_request",
        ...
        "deposit_evidence": {
            "offer_prior_turn_refs": ["chat_1"],
            "supporting_key": "effect",
            "supporting_refs": ["chat_2"],
            "current_intent_refs": ["current_message"],
        },
    }
...
def test_v3_prompts_treat_customer_reply_as_open_channel_without_forcing_pause_cases() -> None:
    assert "抓住客户开口后的 5–10 轮窗口" in PARALLEL_REPLY_SYSTEM_PROMPT
    ...
    assert "明确停止" in PARALLEL_REPLY_SYSTEM_PROMPT
...
def test_parallel_reply_prompt_keeps_store_delivery_fact_based() -> None:
    ...
    assert "本轮门店结果" in prompt
    assert "权威支付订单" in prompt
...
def test_parallel_reply_prompt_separates_activity_from_deposit() -> None:
    ...
    assert "客户尚未进入付款讨论时，第一次介绍活动或价格只讲活动和价值" in prompt
    assert "不发卡" in prompt
    assert "Reply 可采用、组合或忽略全部候选" in PARALLEL_CONTENT_GATE_SYSTEM_PROMPT
...
def test_parallel_reply_requires_a_prior_offer_and_current_action_signal_for_payment() -> None:
    ...
    assert "地址、效果或卡点中的一项真实承接" in prompt
    assert "报名/预约/付款信号" in prompt
    assert "已付后只收【已付登记】仍缺信息" in prompt
    assert "订单不是前置" in prompt
...
def test_parallel_reply_prompt_forbids_placeholder_commit_actions() -> None:
    ...
    assert "只允许 add_customer_mobile 和 create_work_order" in prompt
    assert "仅在权威已付且输入给出完整写入事实时输出" in prompt
    assert "参数及 evidence_refs 必须来自输入" in prompt


def test_parallel_reply_prompt_requires_visible_reply_when_parallel_inputs_are_empty() -> None:
    prompt = PARALLEL_REPLY_SYSTEM_PROMPT

    assert '"reply_messages"' in prompt
    assert "reply_messages：本轮真正发送的全部消息，至少一条" in prompt
    assert "不成立就省略" in prompt
...
def test_parallel_reply_prompt_keeps_paid_registration_fact_without_slot_script() -> None:
    ...
    assert "已付后只收【已付登记】仍缺信息" in prompt
    assert "已付" in prompt and "不发卡" in prompt
...
def test_parallel_reply_prompt_requires_full_deposit_explanation_before_unpaid_registration() -> None:
    ...
    payment_facts = _render_authoritative_facts(rules, topic_ids=["payment"])
    assert "每位先付10元" in payment_facts
    assert "到店抵扣" in payment_facts
    assert "258元" in payment_facts
    assert "未做或不满意可退" in payment_facts
    assert "更早活动介绍" in prompt
    assert "报名/预约/付款信号" in prompt


def test_parallel_prompts_keep_paid_decision_in_reply_not_gate() -> None:
    assert "不输出客户话术、销售动作" in PARALLEL_CONTENT_GATE_SYSTEM_PROMPT
    assert "已付后只收【已付登记】仍缺信息" in PARALLEL_REPLY_SYSTEM_PROMPT
    assert "已付" in PARALLEL_REPLY_SYSTEM_PROMPT and "不发卡" in PARALLEL_REPLY_SYSTEM_PROMPT
...
def test_parallel_gate_uses_history_only_to_rank_optional_assets() -> None:
    ...
    assert "真实交付记录" in prompt
    assert "Reply 可采用、组合或忽略全部候选" in prompt
    assert "不判断客户类型、心理、意向等级、成交阶段或固定主线" in prompt
    assert "已交付资产默认不重发" in prompt
    assert "依赖未满足" in prompt
    assert "direct" in prompt
...
def test_parallel_gate_filters_channel_incompatible_asset_but_reply_owns_action() -> None:
    ...
    assert "付款渠道冲突时不能提名收款资产" in PARALLEL_CONTENT_GATE_SYSTEM_PROMPT


def test_parallel_reply_prompt_forbids_reoffering_delivered_content() -> None:
    assert "已发送素材默认不重复" in PARALLEL_REPLY_SYSTEM_PROMPT
    assert "直接交付，不先问客户要不要看" in PARALLEL_REPLY_SYSTEM_PROMPT
...
def test_parallel_unverified_paid_claim_metadata_does_not_block_text_only_reply() -> None:
    ...
    assert "付款渠道冲突时不能提名收款资产" in PARALLEL_CONTENT_GATE_SYSTEM_PROMPT
    assert "不决定推进、暂停、成交或下一步" in PARALLEL_CONTENT_GATE_SYSTEM_PROMPT
...
def test_parallel_content_media_materializer_uses_selected_content_as_adoption_signal() -> None:
    state = {
        **_parallel_state("价格怎么样"),
        "evidence_join": {
            "content_candidates": [
    ...
    assert materialized_ids == ["s10_activity_intro"]
    assert messages == [
        {"type": "text", "order": 1, "content": "活动价是268元。"},
        {"type": "image", "order": 2, "content": "https://example.invalid/activity.jpg"},
    ]
...
def test_parallel_reply_materializes_selected_passive_media_without_repair() -> None:
    ...
    assert output["reply_messages"] == [
        {"type": "text", "order": 1, "content": "活动规则给您说明清楚了。"},
...
def test_parallel_reply_recovery_keeps_complete_gate_candidate_evidence() -> None:
    ...
    assert "素材 s10_activity_intro" in messages[1]["content"]
    assert "在 selected_content_ids 记录对应 ID" in messages[1]["content"]
    assert "候选图片和视频按 ID 原样交付" in messages[1]["content"]
    assert "used_fact_refs" not in messages[1]["content"]


def test_parallel_reply_context_describes_activity_asset_and_registration_without_private_values() -> None:
    payload = {
        "evidence": {
            "shared_context": {
                "current_time": {},
            ...
            "semantic_route": {"relevant_fact_topic_ids": ["payment", "registration"]},
            "content_candidates": [
                {
                    "content_id": "s10_activity_intro",
                    "name": "活动介绍",
                    ...
                    "messages": [
                        {"type": "text", "content": "活动价268元"},
                        {"type": "image", "content": "https://example.test/activity.png"},
                    ],
                },
                {
                    "content_id": "s10_deposit_close",
                    ...
                    "delivery_observation": {"sent_count": 0},
                    "messages": [
                        {"type": "text", "content": "预约金说明"},
                        {"type": "image", "content": "https://example.test/deposit.png"},
                    ],
                },
                {
                    "content_id": "s10_objection_resolution",
                    "name": "收费顾虑处理",
                    "asset_role": "objection_support",
                    "purpose": "解释费用顾虑",
                    "delivery_observation": {"sent_count": 0},
                    "messages": [{"type": "text", "content": "费用会提前说清"}],
                },
            ],
        },
        "payment_channel_availability": {
        ...
        "registration_fact_status": {
            "authoritative_paid": True,
            "collected_fields": ["customer_name", "customer_mobile"],
            "missing_fields": ["arrival_intent"],
            "confirmed_store": {"store_name": "荆州区店"},
        },
    }

    prompt = build_parallel_reply_messages(payload, json_dumps=json.dumps)[1]["content"]

    assert "首次完整活动或价格介绍的配套凭证" in prompt
    assert "活动文字 + 活动宣传图" in prompt
    assert "当前销售接触尚未发送" in prompt
    assert "不能替代 activity_offer" in prompt
    assert "本素材不能替代活动凭证" in prompt
    assert "收款二维码：未提供" in prompt
    assert "预约金金额、到店抵扣、尾款和可退条件，四项不能省略" in prompt
    assert "确认门店：荆州区店" in prompt
    assert "仍缺：宽松到店意向" in prompt
...
def test_v3_semantic_router_replaces_gate_and_general_tool_planner() -> None:
    class _SemanticRouter:
        async def route(self, *, shared_context, sequence_result=None, taxonomy_result=None, force_store_required=False):
            assert shared_context == {}
            assert sequence_result == {}
            assert taxonomy_result == {}
    ...
    router_trace = result["trace"][0]["tool_calls"][0]["output"]
    assert router_trace["model_usage"]["model"] == "deepseek-v4-flash"
...
def test_gate_prompt_declares_parallel_candidate_only_boundary() -> None:
    ...
    assert "内容证据检索器" in PARALLEL_CONTENT_GATE_SYSTEM_PROMPT
    assert "不按关键词、场景 ID 或配置顺序匹配" in PARALLEL_CONTENT_GATE_SYSTEM_PROMPT
    assert "活动介绍与预约金是不同资产" in PARALLEL_CONTENT_GATE_SYSTEM_PROMPT
...
def test_v3_distance_objection_uses_completed_search_and_switches_value_dimension() -> None:
    ...
    assert "换一个尚未重复的效果、活动价值、真实案例或行动成本角度" in reply_prompt
    assert "应换证据或价值维度" in reply_prompt
    assert "其他常去城市" not in reply_prompt
    assert "跟进序列说明为什么采取某个动作" in reply_prompt
