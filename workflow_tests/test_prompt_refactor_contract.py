...
def test_planner_prompt_is_intent_driven_and_keeps_business_boundaries() -> None:
    ...
    for business_rule in [
        ...
        "活动已说明后客户说“报名/怎么报名/怎么付费”",
        "普通单人报名、预约、付款问题不得用它阻断10元发卡",
        "发卡前置是活动报价已完成/已铺垫",
        "短消息必须承接紧邻的未完动作",
...
def test_sop_gate_prefers_activity_pack_for_activity_and_payment_questions() -> None:
    ...
    for marker in [
        ...
        "s10_activity_intro",
        "不要只返回 `ai_only` 让普通 AI 半套解释活动或空泛解释",
        "不能让客户先读完整长文后才找到答案",
        "recent_sop_delivery_evidence",
        "禁止再次发送完整案例包",
        "活动已经铺垫后再交 `ai_only` 给 Planner 处理发卡和交易事实",
        "不能选 `s10_objection_resolution`、`s10_deposit_close` 或任何 SOP 包来替代交易链路",
        "普通“那怎么报名/怎么付费/我参加”不是收费顾虑，也不是人数确认任务",
        "不能先问“自己一位参加吗/几位参加/按人数登记”",
        "普通单人活动承接只说",
        "普通活动确认，不是收费顾虑",
...
def test_outreach_prompts_require_multi_angle_assets_and_locked_send_structure() -> None:
    for marker in [
        "core_barrier",
        "emotional_need",
        "plan_arc",
