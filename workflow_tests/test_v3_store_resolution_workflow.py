...
from app.graph.nodes.action_module_outputs import (
    ...
    _requested_store_detail_status,
    _reuse_previous_store_delivery,
...
from app.graph.nodes.action_nodes import (
    ...
    _unanchored_geocode_requires_confirmation,
)
...
def test_same_destination_final_store_delivery_is_reused_without_another_card() -> None:
    state = {
        "history_events": [
            {
    ...
    reused = _reuse_previous_store_delivery(state, resolution)

    assert reused["status"] == "reuse_confirmed_store"
    assert reused["delivery_store_ids"] == []
    assert reused["already_delivered_store_ids"] == ["601"]


def test_changed_destination_does_not_reuse_previous_store_delivery() -> None:
    state = {
        "history_events": [
            {
                "event_type": "store_address_sent",
    ...
    assert _reuse_previous_store_delivery(state, resolution) == resolution


def test_v3_store_destination_contract_keeps_city_conflicts_and_arrival_guidance_explicit() -> None:
    assert "广州惠州" in STORE_DESTINATION_RESOLVER_SYSTEM_PROMPT
    assert "arrival_guidance" in STORE_DESTINATION_RESOLVER_SYSTEM_PROMPT
    normalized, violations = _normalize_resolution(
        {
            "request_kind": "store_detail",
            "destination_query": "厦门百星湖里店",
            "destination_precision": "poi",
            "administrative_context": {"city": "厦门市"},
    ...
    assert violations == []
    assert normalized["detail_kind"] == "arrival_guidance"


def test_v3_requested_store_detail_status_distinguishes_public_address_from_room_fact() -> None:
    destination = {"detail_kind": "arrival_guidance"}

    kind, available = _requested_store_detail_status(
        destination,
        [{"store_id": "601", "store_address": "萤火虫大厦"}],
        delivery_store_ids=["601"],
    )
    assert (kind, available) == ("arrival_guidance", False)

    kind, available = _requested_store_detail_status(
        destination,
        [{"store_id": "601", "store_address": "萤火虫大厦", "floor": "3楼", "room": "301"}],
        delivery_store_ids=["601"],
    )
    assert (kind, available) == ("arrival_guidance", True)
...
def test_v3_resolver_admin_fact_keeps_complete_visible_city_scope() -> None:
    stores = [
        {
            "store_id": str(index),
            ...
            "store_address": f"\u957f\u6c99\u5e02 test road {index}",
            "location": f"112.{index:03d},28.{index:03d}",
            "store_fact_integrity": "valid",
        }
    ...
    assert result["status"] == "ok"
    assert result["candidate_store_count"] == 75
    assert len(result["candidate_stores"]) == 75
...
def test_destination_fallback_does_not_geocode_arbitrary_customer_text() -> None:
    state = {
        "normalized_content": "具体地址呢，门牌号多少",
        "shared_context": {
            "current_message": {
                "message_type": "text",
                "content": "具体地址呢，门牌号多少",
    ...
    result = asyncio.run(
        resolve_active_store_destination(
            model_client=None,
            state=state,
            tool={"name": "resolve_customer_store", "purpose": "store_detail"},
        )
    )

    assert result["resolver_status"] == "model_unavailable"
    assert result["destination_query"] == ""
    assert result["needs_clarification"] is True
    assert result["geocode_before_clarification"] is False


def test_destination_fallback_preserves_router_grounded_hint() -> None:
    state = {
        "normalized_content": "这边有店吗",
        "shared_context": {
            "current_message": {"message_type": "text", "content": "这边有店吗"},
            "conversation": [],
            "authoritative_facts": {},
        },
    }
    ...
    assert result["destination_query"] == "武汉汉口"
    assert result["needs_clarification"] is False
    assert result["geocode_before_clarification"] is True
...
def test_geocode_candidate_accepts_county_level_city_returned_as_district() -> None:
    result = _first_geocode_candidate(
        [
            {
                "formatted_address": "\u6e56\u5317\u7701\u8346\u5dde\u5e02\u6d2a\u6e56\u5e02",
                "province": "\u6e56\u5317\u7701",
                "city": "\u8346\u5dde\u5e02",
                "district": "\u6d2a\u6e56\u5e02",
                "location": "113.4700,29.8100",
            }
        ],
        expected_admin={"province": "\u6e56\u5317\u7701", "city": "\u6d2a\u6e56\u5e02"},
        query="\u6d2a\u6e56",
    )

    assert result["city"] == "\u8346\u5dde\u5e02"
    assert result["district"] == "\u6d2a\u6e56\u5e02"
    assert result["expected_admin_conflict"] is False
    assert result["expected_admin_hierarchy_shift"] is True
    assert result["location"] == "113.4700,29.8100"


def test_geocode_candidate_accepts_functional_zone_in_formatted_address() -> None:
    result = _first_geocode_candidate(
        [
            {
                "formatted_address": "\u9655\u897f\u7701\u897f\u5b89\u5e02\u957f\u5b89\u533a\u897f\u54b8\u65b0\u533a\u6ca3\u4e1c\u65b0\u57ce",
                "province": "\u9655\u897f\u7701",
                "city": "\u897f\u5b89\u5e02",
                "district": "\u957f\u5b89\u533a",
                "location": "108.774632,34.262640",
            }
        ],
        expected_admin={
            "province": "\u9655\u897f\u7701",
    ...
    assert result["district"] == "\u957f\u5b89\u533a"
    assert result["expected_admin_conflict"] is False
    assert result["location"] == "108.774632,34.262640"
...
def test_store_workflow_does_not_rank_same_city_wrong_district_geocode_result() -> None:
    class _Model:
        available = True
        ...
        async def chat_json(self, messages, **kwargs):
            ...
            return {
                "request_kind": "nearest",
                "destination_query": "武汉市硚口区古田四路工业园",
                "destination_precision": "poi",
                "administrative_context": {
                    "province": "湖北省",
                    "city": "武汉市",
    ...
    class _Client:
        ...
        async def run_workflow(self, workflow_id, parameters):
            ...
            return {
                "data": [
                    {
                        "formatted_address": "湖北省武汉市武昌区古田四路工业园",
                        "province": "湖北省",
                        "city": "武汉市",
                        "district": "武昌区",
                        "location": "114.3300,30.5500",
                    }
                ]
            }
    ...
    stores = [
        ...
        },
        {
            ...
            "city": "武汉市",
            "district": "江岸区",
            "store_address": "湖北省武汉市江岸区测试路",
            "location": "114.3000,30.6100",
            "store_fact_integrity": "valid",
        },
    ]
    ...
    result = asyncio.run(
        _resolve_customer_store_workflow(
            ...
            {
                "normalized_content": "武汉市硚口区古田四路工业园附近哪家近",
                "shared_context": {
                    "current_message": {
                        "message_type": "text",
                        "content": "武汉市硚口区古田四路工业园附近哪家近",
                    },
                    "conversation": [],
                    "authoritative_facts": {},
                },
    ...
    assert result["status"] == "geocode_query_conflict"
    assert result["customer_store_lookup"]["candidate_stores"] == []
    assert result["customer_store_lookup"]["trusted_origin"]["available"] is False
    assert "distance_calculate" not in result
...
def test_composite_store_workflow_geocodes_query_before_asking_for_clarification() -> None:
    ...
    assert result["customer_store_lookup"]["source"] == "poi_to_geocode_unanchored_result"
    assert result["customer_store_lookup"]["trusted_origin"]["available"] is False


def test_clarification_hint_does_not_override_matching_geocode_admin_region() -> None:
    assert (
        _unanchored_geocode_requires_confirmation(
            ...
            query="通城",
            geocode={
                "location": "113.817,29.245",
                "formatted_address": "湖北省咸宁市通城县",
                "province": "湖北省",
                "city": "咸宁市",
                "district": "通城县",
            },
...
def test_clarification_hint_still_rejects_geocode_without_query_anchor() -> None:
    assert (
        _unanchored_geocode_requires_confirmation(
            {"destination_needs_clarification": True},
            {"customer_store_knowledge": {"stores": []}},
            query="东坑",
            geocode={
                "location": "120.730,31.270",
                "formatted_address": "江苏省苏州市工业园区",
                "province": "江苏省",
                "city": "苏州市",
                "district": "工业园区",
            },
...
def test_composite_store_workflow_rejects_unscoped_geocode_without_place_anchor() -> None:
    class _Model:
        available = True
    ...
    class _Client:
        ...
        async def run_workflow(self, workflow_id, parameters):
            ...
            return {
                "data": {
                    "formatted_address": "福建省漳州市平和县测试停车场",
                    "province": "福建省",
                    "city": "漳州市",
                    "district": "平和县",
                    "location": "117.3000,24.3000",
                }
            }
    ...
    result = asyncio.run(
        _resolve_customer_store_workflow(
            {"name": "resolve_customer_store", "purpose": "nearest"},
            {
                "normalized_content": "九华山风景区山脚停车场附近哪家店近",
                "shared_context": {
                    "current_message": {
                        "message_type": "text",
                        "content": "九华山风景区山脚停车场附近哪家店近",
                    },
                    "conversation": [],
                    "authoritative_facts": {},
                },
    ...
    assert lookup["trusted_origin"]["available"] is False
    assert lookup["location_evidence"].get("city") in (None, "")
...
def test_store_destination_prompt_treats_explicit_cross_city_area_as_correction() -> None:
    assert "它与历史地点跨城市，只说明客户改了查询地点" in (
        STORE_DESTINATION_RESOLVER_SYSTEM_PROMPT
    )
    assert "历史在“武汉沌口”，当前明确改口“岳麓区”" in (
        STORE_DESTINATION_RESOLVER_SYSTEM_PROMPT
    )
