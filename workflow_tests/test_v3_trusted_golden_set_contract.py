...
def test_v3_golden_policy_allows_sales_expression_without_weakening_hard_facts() -> None:
    policy = _load_fixture()["sales_expression_policy"]

    assert "一般性社会证明和客户经验" in policy["approved_general_expression"]
    assert "赞美、幽默、共情和价值类比" in policy["approved_general_expression"]
    assert "价格、支付、退款和活动权益" in policy["hard_fact_boundaries"]
    assert "个体效果保证或个体安全保证" in policy["forbidden_expansion"]
